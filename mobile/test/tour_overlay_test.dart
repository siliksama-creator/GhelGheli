...
void main() {
  ...
  testWidgets('تور خودش باز می‌شود، پردهٔ تار می‌آید و کرش نمی‌کند',
    ...
    // کارت تازه‌باز در فازِ «پیشنهاد» است: صدا با اولین لمسِ کاربر شروع
    // می‌شود (سیاستِ پخشِ خودکار)، پس دکمهٔ اصلی «شروع آموزش» است و
    // «پخش دوباره» — که فقط بعد از شروعِ پخش می‌آید — نباید باشد.
    //
    ...
    expect(find.text('شروع آموزش'), findsOneWidget,
        reason: 'دکمهٔ اصلیِ کارت در شروعِ تور «شروع آموزش» نیست');
    expect(find.text('صدا روشن'), findsOneWidget,
        reason: 'دکمهٔ صدا رندر نشد — ردیفِ دکمه‌ها نمی‌آید');
    expect(find.text('پخش دوباره'), findsNothing,
        reason: 'قبل از شروعِ پخش نباید «پخش دوباره» باشد');
