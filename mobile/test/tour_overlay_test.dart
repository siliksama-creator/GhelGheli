...
//   * «بعدی» همیشه هست (حتی اگر صدا نیامده باشد) و جلو می‌رود
...
void main() {
  ...
  testWidgets('تور خودش باز می‌شود، پردهٔ تار می‌آید و کرش نمی‌کند',
    ...
    // پردهٔ سایه (چهار پنل) — یعنی «بقیهٔ صفحه تیره شد».
    // تار نمی‌شود: تارکردنِ بخش‌هایی که توضیح داده نمی‌شوند آموزش را گیج
    // می‌کرد (خواستهٔ مالک، ۵ مهر ۱۴۰۵).
    expect(find.byType(ColoredBox), findsWidgets,
        reason: 'پنلِ سایه رندر نشد');
    expect(tester.takeException(), isNull);

    // دکمهٔ اصلی **همیشه** «بعدی» است — نه «شروع آموزش». چرا: وقتی نقشِ
    // این دکمه با فازِ پخش عوض می‌شد، در فازهای «پیشنهاد»/«در انتظارِ
    // اجازه» هیچ راهی برای رفتن به بخشِ بعد نمانده بود و کاربر گیر
    // می‌کرد («تور آنلوردینگ در موبایل دکمهٔ بعدی نداشت»).
    //
    // نکته: همین دو دکمهٔ ردیفِ دوم (`OutlinedButton`) ویجت‌هایی بودند که
    // باگِ «عرضِ بی‌نهایت» را می‌ترکاندند؛ پس وجودِ «صدا روشن» خودش
    // سنجهٔ رفعِ آن باگ هم هست.
    expect(find.text('بعدی'), findsOneWidget,
        reason: 'دکمهٔ اصلی باید همیشه «بعدی» باشد');
    expect(find.text('صدا روشن'), findsOneWidget,
        reason: 'دکمهٔ صدا رندر نشد — ردیفِ دکمه‌ها نمی‌آید');
    expect(find.textContaining('پخش'), findsOneWidget,
        reason: 'دکمهٔ پخشِ صدا در ردیفِ دوم نیست');
