...
class _AdminSupportState extends State<AdminSupport> {
  ...
  Future<void> _setClosed(bool close) async {
    ...
    // ═══════════════════════════════════════════════════════════════════
    //
    // بالای همین تابع یک `await showDialog(...)` هست. دیالوگ می‌تواند
    // چند ثانیه باز بماند و در آن فاصله مدیر ممکن است تبِ دیگری را
    // بزند یا برگردد — یعنی این State پیش از رسیدن به این خط dispose
    // شده باشد. آن‌وقت setState پرتاب می‌کند:
    //
    //     setState() called after dispose()
    //
    // در ریلیز این یک استثنای مدیریت‌نشده است، نه یک هشدارِ بی‌خطر.
    if (!mounted) return;
    setState(() => _busy = true);
    try {
