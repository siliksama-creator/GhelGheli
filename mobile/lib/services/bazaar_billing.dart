...
// ═══════════════════════════════════════════════════════════════════════════
//
//   ۱. POST /api/shop/items/:id/buy   → سفارش pending + orderId + productId
//      (یا /api/shop/plus)               قیمت را سرور از دیتابیس می‌خواند
//   ۲. Poolakey.purchase(productId)   → purchaseToken از بازار
//   ۳. POST /api/purchase/verify      → سرور توکن را از API بازار چک
//                                        می‌کند و تازه بعد تحویل می‌دهد
//
// مرحلهٔ ۳ حیاتی است: اگر کلاینت خودش می‌گفت «پرداخت شد، تحویل بده»، هر
// کسی با یک اپ دست‌کاری‌شده می‌توانست رایگان صاحب همه‌چیز شود. سرور هرگز
// به کلاینت اعتماد نمی‌کند و مبلغ را هم خودش از دیتابیس تعیین می‌کند.
//
// کیف پول در این مسیر هیچ نقشی ندارد: خرید ۱۰۰٪ از بازار است و موجودی
// کیف پول فقط برای برداشت نقدی کاربر است.
import 'dart:async';
