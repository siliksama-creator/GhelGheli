...
class _GhelGheliAppState extends State<GhelGheliApp> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: 'قل‌قلی', debugShowCheckedModeBanner: false, locale: const Locale('fa'), supportedLocales: const [Locale('fa')],
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  @override Widget build(BuildContext c) => Scaffold(body: Container(decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xff0b2b4f), Color(0xff07111f)])), child: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: CardShell(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    ...
    if (registerMode) ...[const SizedBox(height: 10), TextField(controller: name, decoration: const InputDecoration(labelText: 'نام مستعار')), const SizedBox(height: 10), TextField(controller: code, decoration: InputDecoration(labelText: 'کد تایید', suffixIcon: TextButton(onPressed: () => run(() async { final r = await widget.api.post('/api/auth/request-otp', {'mobile': mobile.text, 'purpose': 'register'}); msg = 'کد تایید ارسال شد ${r['devCode'] ?? ''}'; }), child: const Text('ارسال کد'))))],
    ...
    FilledButton.icon(icon: Icon(adminMode ? Icons.admin_panel_settings : Icons.login), onPressed: loading ? null : () => run(() async {
      ...
      else if (registerMode) { await widget.api.post('/api/auth/verify-otp', {'mobile': mobile.text, 'code': code.text, 'purpose': 'register'}); final r = await widget.api.post('/api/auth/register', {'mobile': mobile.text, 'password': pass.text, 'nickname': name.text}); await widget.api.saveToken(r['token']); }
  ...
  Widget build(BuildContext c) {
    ...
    return RefreshIndicator(onRefresh: load, child: ListView(padding: const EdgeInsets.all(18), children: [
      ...
      CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const Text('ثبت کد کارت', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
...
class HeroHeader extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) {
    ...
    return Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(borderRadius: BorderRadius.circular(34), gradient: const LinearGradient(colors: [Color(0xff00d49a), Color(0xff1c78ff)]), boxShadow: [BoxShadow(color: Colors.tealAccent.withOpacity(.25), blurRadius: 35)]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _ChatPageState extends State<ChatPage> { final text = TextEditingController(); List msgs = []; io.Socket? socket; String? error; @override void initState() { super.initState(); load(); socket = io.io(ApiClient.defaultBaseUrl, io.OptionBuilder().setTransports(['websocket']).setAuth({'token': widget.api.token}).build()); socket!.on('chat:new', (m) => setState(() => msgs = [...msgs, m])); } @override void dispose() { socket?.dispose(); super.dispose(); } Future<void> load() async { try { final cfg = await widget.api.get('/api/chat/config'); if (cfg['eligible'] != true) { setState(() => error = 'برای چت باید حداقل ${faNum(cfg['minLifetimePoints'])} امتیاز تاریخی داشته باشید. امتیاز شما: ${faNum(cfg['userLifetimePoints'])}'); return; } final d = await widget.api.get('/api/chat/messages'); setState(() { msgs = d; error = null; }); } catch (e) { setState(() => error = apiError(e)); } } @override Widget build(BuildContext c) => Column(children: [if (error != null) Expanded(child: Center(child: CardShell(child: Text(error!, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold))))) else Expanded(child: ListView.builder(padding: const EdgeInsets.all(14), itemCount: msgs.length, itemBuilder: (_, i) { final m = msgs[i]; return ListTile(title: Text(m['nickname'] ?? m['first_name'] ?? 'کاربر'), subtitle: Text(m['message_text'] ?? ''), trailing: IconButton(icon: const Icon(Icons.flag), onPressed: () => widget.api.post('/api/chat/messages/${m['id']}/report', {}))); })), SafeArea(child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [Expanded(child: TextField(controller: text, enabled: error == null, decoration: const InputDecoration(hintText: 'پیام گروهی...'))), IconButton.filled(onPressed: error != null ? null : () { socket?.emitWithAck('chat:send', text.text, ack: (r) { if (r['ok'] == true) text.clear(); else ScaffoldMessenger.of(c).showSnackBar(SnackBar(content: Text('${r['error'] ?? 'ارسال نشد'}'))); }); }, icon: const Icon(Icons.send))])))]); }
...
class _AdminShellState extends State<AdminShell> {
  ...
  Widget build(BuildContext c) => Scaffold(
    appBar: AppBar(title: Text('مدیریت قل‌قلی — ${titles[idx]}'), actions: [IconButton(onPressed: widget.onTheme, icon: Icon(widget.dark ? Icons.light_mode : Icons.dark_mode)), IconButton(onPressed: widget.onLogout, icon: const Icon(Icons.logout))]),
...
class _AdminRewardsState extends State<AdminRewards> {
  ...
  @override Widget build(BuildContext c) => ListView(padding: const EdgeInsets.all(16), children: [
    CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      DropdownButtonFormField(value: type, items: const [DropdownMenuItem(value: 'cash', child: Text('نقدی')), DropdownMenuItem(value: 'physical', child: Text('فیزیکی'))], onChanged: (v) => setState(() => type = v!)),
...
class _AdminSettingsState extends State<AdminSettings> { final chatMin = TextEditingController(); String? msg; @override void initState() { super.initState(); load(); } Future<void> load() async { final d = await widget.api.get('/api/admin/settings/chat'); chatMin.text = '${d['minLifetimePoints'] ?? 0}'; if (mounted) setState(() {}); } Future<void> save() async { try { final r = await widget.api.patch('/api/admin/settings/chat', {'minLifetimePoints': int.tryParse(chatMin.text) ?? 0, 'reason': 'تنظیم از اپ مدیریت'}); msg = r['message'] ?? 'ذخیره شد'; } catch (e) { msg = apiError(e); } if (mounted) setState(() {}); }
  @override Widget build(BuildContext c) => ListView(padding: const EdgeInsets.all(16), children: [CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [const Text('تنظیمات چت کاربران', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18)), const SizedBox(height: 8), const Text('کاربر فقط وقتی می‌تواند وارد چت شود و پیام بفرستد که حداقل این مقدار امتیاز تاریخی داشته باشد. امتیاز تاریخی بعد از دریافت جایزه صفر نمی‌شود.'), TextField(controller: chatMin, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'حداقل امتیاز تاریخی برای چت')), const SizedBox(height: 10), FilledButton.icon(onPressed: save, icon: const Icon(Icons.save), label: const Text('ذخیره محدودیت چت')), if (msg != null) Padding(padding: const EdgeInsets.only(top: 10), child: Text(msg!))]))]); }
...
class _AdminAdminsState extends State<AdminAdmins> { List admins = [], logs = []; final u = TextEditingController(), p = TextEditingController(); String role = 'support'; @override void initState() { super.initState(); load(); } Future<void> load() async { admins = await widget.api.get('/api/admin/admins'); logs = await widget.api.get('/api/admin/audit-log'); setState(() {}); } Future<void> add() async { await widget.api.post('/api/admin/admins', {'username': u.text, 'password': p.text, 'role': role}); u.clear(); p.clear(); await load(); }
  @override Widget build(BuildContext c) => ListView(padding: const EdgeInsets.all(16), children: [CardShell(child: Column(children: [const Text('ادمین جدید'), TextField(controller: u, decoration: const InputDecoration(labelText: 'نام کاربری')), TextField(controller: p, decoration: const InputDecoration(labelText: 'رمز عبور')), DropdownButtonFormField(value: role, items: const [DropdownMenuItem(value: 'super_admin', child: Text('مدیر کل')), DropdownMenuItem(value: 'support', child: Text('پشتیبان')), DropdownMenuItem(value: 'observer', child: Text('ناظر'))], onChanged: (v) => setState(() => role = v!),), FilledButton(onPressed: add, child: const Text('ایجاد ادمین'))])), CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('ادمین‌ها'), ...admins.map((a) => ListTile(title: Text(a['username']), subtitle: Text(a['role'])))])), CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Audit Log'), ...logs.take(80).map((l) => Text('${l['username'] ?? 'سیستم'} — ${l['action']} — ${l['created_at']}'))]))]); }
