...
class _AdminSettingsState extends State<AdminSettings> {
  final chatMin = TextEditingController(); final cooldown = TextEditingController();
  ...
  @override void initState() { super.initState(); load(); }
  Future<void> load() async { final c = await widget.api.get('/api/admin/settings/chat'); chatMin.text = '${c['minLifetimePoints'] ?? 0}'; cooldown.text='${c['messageCooldownSeconds'] ?? 5}'; final s=await widget.api.get('/api/admin/settings/sms'); provider.text=s['provider']??''; sender.text=s['sender']??''; apiKey.text=s['apiKeyMasked']??''; pattern.text=s['patternCode']??''; smsEnabled=s['enabled']==true; smsTest=s['testMode']!=false; if (mounted) setState(() {}); }
  Future<void> saveChat() async { try { final r = await widget.api.patch('/api/admin/settings/chat', {'minLifetimePoints': int.tryParse(chatMin.text) ?? 0, 'messageCooldownSeconds': int.tryParse(cooldown.text) ?? 5, 'reason': 'تنظیم از اپ مدیریت'}); msg = r['message'] ?? 'ذخیره شد'; } catch (e) { msg = apiError(e); } if (mounted) setState(() {}); }
