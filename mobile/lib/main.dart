...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Widget build(BuildContext c) => Scaffold(
    body: Stack(children: [
      Positioned.fill(child: Image.asset('assets/brand/login_hero.webp', fit: BoxFit.cover)),
  ...
  Widget build(BuildContext c) {
    ...
    return RefreshIndicator(onRefresh: load, child: ListView(padding: const EdgeInsets.all(18), children: [
      ...
      CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ClipRRect(borderRadius: BorderRadius.circular(22), child: Image.asset('assets/brand/card_pack_banner.webp', height: 132, fit: BoxFit.cover)), const SizedBox(height: 12), const Text('ثبت کد کارت های قلقلی', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
...
class _RewardsPageState extends State<RewardsPage> { List rewards = []; @override void initState() { super.initState(); load(); } Future<void> load() async => setState(() {}); @override Widget build(BuildContext c) => FutureBuilder(future: widget.api.get('/api/rewards'), builder: (c, s) { if (!s.hasData) return const Center(child: CircularProgressIndicator()); rewards = s.data as List; return ListView(padding: const EdgeInsets.all(18), children: [CardShell(child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[ClipRRect(borderRadius:BorderRadius.circular(22),child:Image.asset('assets/brand/rewards_banner.webp',height:140,fit:BoxFit.cover)),const SizedBox(height:12),const Text('جوایز قلقلی',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900)),const Text('امتیاز جمع کن، جایزه بگیر و پیشرفتت را ببین.') ])),const SizedBox(height:12),...rewards.map((r) => CardShell(child: ListTile(leading: r['image_url'] != null ? ClipRRect(borderRadius:BorderRadius.circular(14), child: Image.network(fullAssetUrl(r['image_url']), width:54, height:54, fit:BoxFit.cover)) : const Icon(Icons.card_giftcard), title: Text(r['name'] ?? '', style: const TextStyle(fontWeight:FontWeight.w900)), subtitle: Text('${faNum(r['required_points'])} امتیاز — ${r['reward_value']}'), trailing: FilledButton.tonal(onPressed: r['eligible'] == true ? () async { await widget.api.post('/api/rewards/${r['id']}/claim', {}); if (mounted) ScaffoldMessenger.of(c).showSnackBar(const SnackBar(content: Text('درخواست جایزه ثبت شد'))); } : null, child: const Text('دریافت'))))).toList()]); }); }
...
class _LeaguePageState extends State<LeaguePage>{Map? d;Timer? t;@override void initState(){super.initState();load();t=Timer.periodic(const Duration(seconds:12),(_)=>load());}@override void dispose(){t?.cancel();super.dispose();}Future<void> load()async{final x=await widget.api.get('/api/league/current');if(mounted)setState(()=>d=x);} @override Widget build(BuildContext c){final entries=List<Map>.from(d?['entries']??[]);final season=d?['season'];final end=season?['ends_at']==null?null:DateTime.parse(season['ends_at']);final left=end==null?'':faNum(end.difference(DateTime.now()).inDays)+' روز تا پایان ماه';final top=entries.take(3).toList();final rest=entries.skip(3).toList();return RefreshIndicator(onRefresh:load,child:ListView(padding:const EdgeInsets.all(18),children:[Container(padding:const EdgeInsets.all(22),decoration:BoxDecoration(borderRadius:BorderRadius.circular(32),gradient:const LinearGradient(colors:[Color(0xff172f56),Color(0xff00d49a)]),boxShadow:[BoxShadow(color:Colors.black.withValues(alpha:.25),blurRadius:28)]),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[ClipRRect(borderRadius:BorderRadius.circular(24),child:Image.asset('assets/brand/league_banner.webp',height:120,width:double.infinity,fit:BoxFit.cover)),const SizedBox(height:12),const Text('لیگ ماهانه قلقلی',style:TextStyle(color:Colors.white,fontWeight:FontWeight.w900,fontSize:26)),const SizedBox(height:6),Text('برترین کاربران تا پایان ماه؛ امتیاز لیگ آخر ماه ریست می‌شود اما امتیاز کلی دست نمی‌خورد.',style:TextStyle(color:Colors.white.withValues(alpha:.9))),const SizedBox(height:12),Chip(label:Text(left.isEmpty?'در حال محاسبه':left))])),const SizedBox(height:16),if(top.isNotEmpty)Row(crossAxisAlignment:CrossAxisAlignment.end,children:top.asMap().entries.map((e){final i=e.key;final r=e.value;return Expanded(child:GestureDetector(onTap:()=>showPublicProfile(c,widget.api,r['user_id']),child:Container(height:i==0?170:140,margin:const EdgeInsets.all(5),padding:const EdgeInsets.all(12),decoration:BoxDecoration(borderRadius:BorderRadius.circular(24),gradient:LinearGradient(colors:i==0?[const Color(0xffffd36b),const Color(0xff7a4d00)]:[Theme.of(c).colorScheme.surfaceContainerHighest,Theme.of(c).colorScheme.surface]),boxShadow:[BoxShadow(color:Colors.black.withValues(alpha:.18),blurRadius:18)]),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[Text(['🥇','🥈','🥉'][i],style:const TextStyle(fontSize:34)),Text(r['nickname']??r['first_name']??'کاربر',maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontWeight:FontWeight.w900)),Text('${faNum(r['points'])} امتیاز')]))));}).toList()),const SizedBox(height:14),...rest.asMap().entries.map((e)=>RankTile(rank:e.key+4,row:e.value,onTap:()=>showPublicProfile(c,widget.api,e.value['user_id']))),if(entries.isEmpty)const CardShell(child:Text('هنوز امتیازی در لیگ ثبت نشده است',textAlign:TextAlign.center))]));}}
...
class _ChatPageState extends State<ChatPage>{
  ...
  @override Widget build(BuildContext c)=>Column(children:[
    CardShell(child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[ClipRRect(borderRadius:BorderRadius.circular(22),child:Image.asset('assets/brand/chat_banner.webp',height:118,fit:BoxFit.cover)),const SizedBox(height:10),const Text('چت روم قلقلی',style:TextStyle(fontWeight:FontWeight.w900,fontSize:18)),const Text('کاربران در این قسمت میتوانند باهم گفتگو کنند. (از الفاظ رکیک و بحث های سیاسی جدا خودداری بشه.)') ])),
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  @override Widget build(BuildContext c) => !loaded ? const Center(child:CircularProgressIndicator()) : ListView(padding: const EdgeInsets.all(18), children: [CardShell(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    ClipRRect(borderRadius: BorderRadius.circular(22), child: Image.asset('assets/brand/profile_banner.webp', height: 130, fit: BoxFit.cover)), const SizedBox(height: 12), const Text('تکمیل پروفایل خصوصی', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
