...
// ── زمان‌بندی ────────────────────────────────────────────────────────────
//
// این عددها «تأخیرِ پیش از صدا» هستند، نه فقط طولِ انیمیشن: صدا **بعد** از
// مکثِ در، سکونِ ناوبری و سفرِ انگشت پخش می‌شود (پایین، `runStep`). با
// مقادیرِ قبلی روی کاغذ ۱٫۵ + ۰٫۵۶ + ۱٫۶۵ ≈ ۳٫۷ ثانیه سکوت بود، و در عمل
// دانلودِ فایلِ صوتی هم روی همین مسیر می‌نشست — یعنی کاربر نزدیکِ چهار
// ثانیه تصویرِ بی‌صدا می‌دید و می‌پرسید «صدا کی می‌آید؟».
//
// کوتاه‌ترشان کردیم بدون اینکه روایت خراب شود: حرکت هنوز دیده می‌شود، فقط
// نفس‌گیر نیست. و چون حالا فایلِ صوتی از قبل پیش‌بار می‌شود (پایین،
// پیش‌بارگذاریِ بخشِ جاری)، زمانِ دانلود کلاً از مسیرِ بحرانی بیرون رفته.
const ADVANCE_MS = 700; // مکثِ بعد از پایانِ صدا
const NAV_SETTLE = 380; // فرصتِ بازشدنِ صفحه/زیرتب/شیتِ تازه
const MAX_WAIT = 2200;  // حداکثر انتظار برای ظهورِ لنگر
const DOOR_MS = 900;    // مکثِ مرحلهٔ «در»: انگشت می‌نشیند و تاچ می‌کند
const ENTER_MS = 900;   // مکثِ سفرِ انگشت از در به هدف
// ── سقفِ کادرِ تمرکز ────────────────────────────────────────────────────
...
export default function Tour({ token, tab, goTab }) {
  ...
  // ── پیش‌بارگذاریِ صدا: بخشِ جاری **و** بعدی ───────────────────────────
  //
  // چرا «جاری» هم: تا پیش از این فقط بخشِ بعد پیش‌بار می‌شد، پس نخستین
  // بخشِ هر تور (و هر بخشی که کاربر با پرش می‌رسید) باید از صفر دانلود
  // می‌شد — و این دانلود دقیقاً بعد از پایانِ انیمیشن شروع می‌شد، یعنی
  // درست همان لحظه‌ای که کاربر منتظرِ شنیدن است.
  //
  // این effect به محضِ تغییرِ `idx` اجرا می‌شود، یعنی هم‌زمان با مکثِ «در»
  // و سفرِ انگشت. وقتی `playCurrent` بالاخره صدا می‌زند، فایل معمولاً در
  // کشِ HTTP است و `play()` فوراً جواب می‌دهد.
  useEffect(() => {
    const list = [data?.steps?.[idx], data?.steps?.[idx + 1]];
    const els = [];
    for (const s of list) {
      if (!s?.audioUrl) continue;
      const a = new Audio();
      a.preload = 'auto';
      a.src = API + s.audioUrl;
      els.push(a);
    }
    return () => {
      for (const a of els) {
        try { a.removeAttribute('src'); a.load(); } catch { /* بی‌اهمیت */ }
      }
    };
  }, [data, idx]);
