...
import { CosmeticAvatarFrame, RESULT_PALETTES, nameColorStyle, profileBackgroundStyle } from '../components/Cosmetics.jsx';
...
function CosmeticPreview({ item }) {
  ...
  if (item.kind === 'name_color') return <div className="shopArtwork shopLiveName">
    <img src="/avatars/avatar_10_crown.webp" alt=""/><div><strong style={nameColorStyle(value)}>hotcat</strong><span>★ عضو پلاس · لول ۷۲</span></div>
  </div>;

  if (item.kind === 'profile_background') return <div className="shopArtwork shopLiveProfile" style={profileBackgroundStyle(value)}>
...
function PlanCard({ plan, activeTier, busy, onBuy }) {
  ...
  return <article className={`shopPlan ${annual ? 'annual' : ''} ${active ? 'active' : ''}`}>
    ...
    <div className="planVisuals" aria-label="نمونه واقعی مزایای پلن">
      ...
      <div className="planNameSwatch" style={nameColorStyle(annual?'mvp_name':'gold_gradient')}>{annual?'MVP':'hotcat'}</div>
...
export default function Shop({ token, reloadProfile }) {
  ...
  return <div className="shopCompact" dir="rtl">
    <style>{`
      ...
      .shopLiveFrame{display:flex;align-items:center;justify-content:center;gap:14px;background:radial-gradient(circle,#1e293b,#030712)}.shopLiveFrameRing{width:92px;height:92px;box-shadow:0 0 22px #38bdf844}.shopLiveFrameRing img{width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid #071522}.shopLiveFrame>div{display:flex;flex-direction:column}.shopLiveFrame span{font-size:18px;font-weight:950;color:#fff}.shopLiveFrame small{font-size:8px;color:#94a3b8;margin-top:3px}.shopLiveName{display:flex;align-items:center;justify-content:center;gap:14px;padding:20px;background:linear-gradient(145deg,#071522,#111827)}.shopLiveName>img{width:62px;height:62px;border-radius:50%;object-fit:cover;border:2px solid #ffffff20}.shopLiveName>div{display:flex;flex-direction:column;align-items:flex-start}.shopLiveName strong{font-size:24px;font-weight:950}.shopLiveName span{font-size:8px;color:#94a3b8;margin-top:5px}
