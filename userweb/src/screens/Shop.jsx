...
const KINDS = [
  ...
  //
  // ⚠️ سرور هم همین دو نوع را از ویترین برداشته (`SHELF_HIDDEN_KINDS` در
  //    `shopService.js`) و خریدِ مستقیمشان را رد می‌کند؛ حذفِ تب فقط برای
  //    تمیزیِ UI است، نه تنها لایهٔ حفاظت.
];
...
export default function Shop({ token, reloadProfile }) {
  ...
  return <div className="shopCompact" dir="rtl">
    <style>{`
      ...
      @media(max-width:720px){.shopHeroTop{align-items:flex-start;flex-wrap:wrap}.shopHero h2{font-size:19px}.shopPlans{grid-template-columns:1fr;overflow-x:visible;padding-bottom:5px}.shopPlan{min-height:0}.shopPlan ul{grid-template-columns:1fr 1fr}.shopCarousel{grid-template-columns:repeat(2,minmax(0,1fr))}.shopWallet{font-size:11px}.shopHero{padding:13px}.shopShelf{padding:10px}}
      ...
      {showPlans && <div className="shopPlans">{(data.plans || []).map((plan) => {
        ...
        return (
          <PlanCard
            ...
            plan={plan}
            activeTier={data.plus?.tier}
            busy={busy === `plus-${plan.billingCycle}`}
            onBuy={buyPlan}
    ...
    {showHistory && <section className="historyPanel">{history.length ? history.map((h) => <div className="historyRow" key={`${h.type}-${h.id}`}><b>{h.name}</b><span>{money(h.price_paid)} · {h.gateway === 'zarinpal' ? 'زرین‌پال · ' : h.gateway === 'wallet' ? 'کیف پول · ' : ''}{new Date(h.purchased_at).toLocaleDateString('fa-IR')}</span></div>) : <span className="muted">هنوز خریدی ثبت نشده است.</span>}</section>}
  </div>;
}
