...
// `primeImageCache` قبلاً همان رشته را مستقیم به `fetch()` می‌داد و مرورگر
// آن را نسبت به **مبدأ صفحه** حل می‌کرد؛ آن زمان روی دامنهٔ user هیچ فایلی
// در `/uploads/` نبود و قاعدهٔ SPA در nginx (`try_files $uri /index.html`)
// به‌جای ۴۰۴، **۲۰۰ با index.html** برمی‌گرداند (۱۴۷۸ بایت، `text/html`).
//
// از اصلاحِ «APIِ هم‌مبدأ» به بعد، nginx روی همان vhostِ وب مسیر `/uploads/`
// را به بک‌اند پروکسی می‌کند؛ پس حلِ مسیرِ نسبی نسبت به مبدأ صفحه دیگر غلط
// نیست — درستِ درست است. `asset()` هم وقتی `VITE_API_BASE` ست شده باشد URL
// را مطلق می‌کند و در حالتِ هم‌مبدأ همان مسیرِ نسبی را برمی‌گرداند.
//
