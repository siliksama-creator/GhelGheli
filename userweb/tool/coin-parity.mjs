...
// ⚠️ پنجرهٔ ۶۰۰ کاراکتری با آمدنِ عددهای `refund`/`fee` (تساویِ پنالتی،
//    ۳ مهر ۱۴۰۵) تنگ شد: بدنهٔ همان payload دو تابعِ کوچکِ کمکی گرفت و
//    کلیدِ `coins` از پنجره بیرون افتاد — یعنی گارد **بی‌دلیل** قرمز شد،
//    نه اینکه قرارداد شکسته باشد. پنجره به ۱۵۰۰ رسید و `coins\s*:` هم
//    دقیق‌تر شد تا «کلمهٔ coins در یک کامنت» گارد را راضی نکند.
ok('سرور coins را در game:settlement می‌فرستد',
  /game:settlement[\s\S]{0,1500}?coins\s*:/.test(backend.engine));
...
//
// 🔴 این حلقه قبلاً اعداد را **هاردکد** کرده بود: `[['دوئل ۱۰۰', 2],
//    ['دوئل ۱۰۰۰', 20], ...]`. یعنی گارد، نسخهٔ منسوخِ جدول را به‌عنوان
//    «درست» قفل کرده بود. وقتی `COIN_TABLE` در دورِ ۲۶ عوض شد و هر سه
//    بازی یکسان شدند، گارد همچنان سبز می‌ماند چون داشت دروغ را با دروغ
//    مقایسه می‌کرد. کاربر در راهنما «۲» می‌دید و در بازی «۱۰» می‌گرفت.
//
...
ok('جدولِ سکهٔ بک‌اند خوانده شد', stakeRows.length > 0);

// هر عددی که بک‌اند واقعاً می‌پردازد باید در هر دو راهنما دیده شود.
const needed = new Set();
for (const [, , win, draw, loss] of stakeRows) {
  needed.add(win); needed.add(draw); needed.add(loss);
}
for (const n of [...needed].sort((a, b) => a - b)) {
  const re = new RegExp(`\\b${n}\\b`);
  ok(`مقدارِ سکهٔ «${n}» (از COIN_TABLE) در هر دو کلاینت آمده`,
    re.test(guideWeb) && re.test(guideAnd));
}

// و برعکس: راهنما نباید عددی نشان بدهد که بک‌اند هرگز نمی‌پردازد.
// (`2` و `20`ِ منسوخ دقیقاً همین بودند.)
// `read()` کامنت‌ها را حذف می‌کند — لازم است، چون کامنتِ توضیحیِ خودِ
// نوار عددهای منسوخ را نقل می‌کند و وگرنه اینجا مثبتِ کاذب می‌سازد.
const rateStrip = {
  web: read('userweb/src/components/CoinRateStrip.jsx'),
  and: read('mobile/lib/widgets/coin_rate_strip.dart'),
};
...
ok('نوارِ نرخِ وب فقط عددهای واقعیِ بک‌اند را نشان می‌دهد',
  webNums.every(n => needed.has(n)));
ok('نوارِ نرخِ اندروید فقط عددهای واقعیِ بک‌اند را نشان می‌دهد',
  andNums.every(n => needed.has(n)));
ok('نوارِ نرخ در هر دو کلاینت دقیقاً یک‌جور است',
  webNums.join(',') === andNums.join(','));

// ═══════════════════════════════════════════════════════════════════════
// فاز ۲ (نقشه‌راه یکپارچه‌سازی): جمله‌ها از قالبِ سرور خوانده می‌شوند
// ═══════════════════════════════════════════════════════════════════════
//
// نسخهٔ قبلیِ این گارد «بازی در ورودی ۱۰۰» را می‌خواست — یعنی **رقمِ**
// هاردکد را قفل کرده بود. با زنده‌شدنِ متن‌ها آن رقم جابه‌جا می‌شود، پس
// گاردِ کهنه دو کار می‌کند: یا محصولِ درست را قرمز می‌کند، یا (بدتر)
// آدم یاد می‌گیرد قرمزی‌اش را نادیده بگیرد.
//
// سنجه‌ها حالا دو چیزند:
//   ۱. هر دو کلاینت جمله را از همان **کلیدِ** live_copy می‌سازند
//      (پس متن واقعاً یکی است و هیچ‌کدام نسخهٔ خودش را ندارد)؛
//   ۲. هیچ‌کدام رقمِ سهمیه/ورودی را در فایلِ راهنما نمی‌نویسند؛ عدد
//      از `economy` و از `live_rules` ساخته می‌شود.
const QUOTA_KEYS = [
  ['هر سه بازی یکسان', 'coinGuide.allEqual'],
  ['ربات و تمرین و لابی سکه ندارند', 'coinGuide.noCoin'],
  ['هرگز کم نمی‌شود', 'coinGuide.neverLost'],
  ['سهمیهٔ روزانه', 'coinGuide.quota'],
  ['سکهٔ ضربه‌زن', 'coinGuide.tapCoins'],
  ['ریست پایانِ فصل', 'coinGuide.league'],
  ['عنوانِ راهنما', 'coinGuide.heading'],
];
for (const [label, key] of QUOTA_KEYS) {
  const used = new RegExp(`['\"]${key}['\"]`);
  ok(`قاعدهٔ «${label}» در هر دو کلاینت از همان کلیدِ live_copy ساخته می‌شود`,
    used.test(guideWeb) && used.test(guideAnd));
}
// رقمِ ورودی در **هیچ‌کدام** از دو فایل نباید بی‌دست‌زدن بیاید؛ رشته‌های
// پایین فقط فول‌بک‌اند و از همان متغیرِ ساخته‌شده از config می‌آیند.
// سه شکلِ مجازِ «ساخته‌شدن از متغیر»:
//   • `ورودی ${fa(stakeLow)}` — وب، رشتهٔ قالبِ فارسی‌شده
//   • `ورودی ${faNum(stakeLow)}` — همان چیزی که dart format می‌سازد
//   • `stakeLabel(…)` — هر دو، وقتی خودِ «ورودی» از قالبِ config می‌آید
// اگر کسی «ورودی ۱۰۰» را دستی در جمله بگذارد، هیچ‌کدام نمی‌خورد و گارد
// قرمز می‌شود. (نسخهٔ اولِ این خط فقط شکلِ وب را می‌شناخت و اندرویدِ
// درست را رد می‌کرد — گاردی که فقط یک کلاینت را ببیند، نیمی از کار است.)
const DYN = /ورودی \$\{(fa|faNum)\(stakeLow\)\}|stakeLabel\(/;
ok('سهمیهٔ روزانه در وب از متغیر ساخته می‌شود، نه از رقمِ سفت',
  DYN.test(guideWeb));
ok('سهمیهٔ روزانه در اندروید از متغیر ساخته می‌شود، نه از رقمِ سفت',
  DYN.test(guideAnd));
// رقمِ فارسیِ «۱۰۰/۱۰۰۰» باید **فقط** در دو جا باشد: در جدولِ `stake`‌ها
// (که از config می‌آید) و در کامنت. در رشته‌هایِ متن، هیچ. این خط
// برگشتِ باگِ دور ۳۲ را نمی‌گذارد: کسی عدد را به متنِ راهنما برگرداند
// و گارد چون کلیدِ live_copy را می‌دید، سبز بماند.
for (const [label, src] of [['وب', guideWeb], ['اندروید', guideAnd]]) {
  const strings = [...src.matchAll(/'([^'\n]*ورودی[^'\n]*)'|"([^"\n]*ورودی[^"\n]*)"|`([^`\n]*ورودی[^`\n]*)`/g)]
    .map(m => m[1] ?? m[2] ?? m[3]);
  const frozen = strings.filter(t => /[۰-۹][۰-۹]*$/.test(t.trim()) && !t.includes('${'));
  ok(`${label}: هیچ رشتهٔ «ورودی ۱۰۰/۱۰۰۰» سفت‌شده نمانده (${strings.length} رشتهٔ بررسی شد)`,
    frozen.length === 0);
}
ok('ورودی‌ها از کلیدِ stakeLabel و از دامنهٔ dailyCoinQuota ساخته می‌شوند',
  /coinGuide\.stakeLabel/.test(guideWeb) && /coinGuide\.stakeLabel/.test(guideAnd)
  && /dailyCoinQuota/.test(guideWeb) && /dailyCoinQuota/.test(guideAnd));
ok('جملهٔ «صفر شدنِ سکه در پایان فصل» در هر دو کلاینت هست',
