...
check('اندروید: خطِ راهنما در حالتِ فشرده پنهان است و نشانِ ×۲ دارد', () => {
  ...
  assert(dart.includes('×۲ دو‌امتیازی'), 'نشانِ راندِ دو‌امتیازی روی کادر نیست');
});

check('هر دو پلتفرم برچسبِ خودِ راند را درشت نشان می‌دهند (نه نامِ خشکِ ویژگی)', () => {
  // وب: <b>{label}</b> که از focus.label بک‌اند می‌آید.
  assert(/<b>\{label\}<\/b>/.test(jsx), 'وب برچسبِ راند را درشت نشان نمی‌دهد');
  // اندروید: bigLabel از label ساخته می‌شود و همان در Textِ درشت می‌نشیند.
  assert(/final bigLabel = label\.trim\(\)\.isNotEmpty/.test(dart),
    'اندروید باید `bigLabel` را از برچسبِ بک‌اند بسازد');
  assert(/Text\(\s*\n?\s*bigLabel,/.test(dart), 'اندروید متنِ درشت را از bigLabel نمی‌گیرد');
  // و هر دو باید نامِ ویژگی را هم داشته باشند (برای برچسبِ دسترس‌پذیری/راهنما).
  for (const k of ['speed', 'goalChance']) {
    assert(jsx.includes(k) && dart.includes(k), `نگاشتِ ${k} در هر دو سمت لازم است`);
  }
});

check('هیچ رشتهٔ کاربرپسندی با نیم‌فاصلهٔ جاافتاده نیست (درسِ ۲۹ شهریور)', () => {
  // ⚠️ همین باگ امروز یک تست را در CI قرمز کرد: بَجِ اندروید «دوامتیازی»
  // (بی‌نیم‌فاصله) نوشته شده بود و بقیهٔ پروژه — بک‌اند، وب، برچسب‌های راند —
  // از «دو‌امتیازی» استفاده می‌کنند. چون دو شکلِ واژه در متن‌های کاربرپسند
  // قاطی می‌شود، این تست کامنت‌ها را حذف می‌کند و شکلِ بی‌نیم‌فاصله را در
  // `jsx` و `dart` ممنوع می‌کند.
  const strip = (src) => src
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/(^|\s)\/\/[^\n]*/g, '$1');
  for (const [name, raw] of [['وب', jsx], ['اندروید', dart]]) {
    const src = strip(raw);
    const bare = 'دو' + 'امتیازی';
    assert(!src.includes(bare),
      `${name}: «${bare}» بدونِ نیم‌فاصله در متنِ کاربرپسند هست (شکلِ درست: دو‌امتیازی)`);
    assert(src.includes('دو‌امتیازی'),
      `${name}: شکلِ نیم‌فاصله‌دارِ «دو‌امتیازی» پیدا نشد`);
  }
});
