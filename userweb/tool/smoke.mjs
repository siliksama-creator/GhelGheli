...
try {
  ...
  if (!TOKEN) {
    ...
    // ── پردهٔ معرفیِ «دوئل طوفان» (۲۹ شهریور) ─────────────────────────────
    // با روشن‌شدنِ مودِ طوفان (مرحلهٔ ۱)، اولین ورود به تمرین یک پردهٔ
    // آموزشِ تمام‌صفحه نشان می‌دهد («دوئل طوفان رسید!»). تا کاربر آن را
    // نبندد، دکمهٔ «ورود به تمرین» زیرِ پرده است و کلیک نمی‌خورد — دقیقاً
    // همان چیزی که این تست را قرمز کرد. اول پرده را می‌بندیم (اگر بود).
    const mayhemIntro = page.locator('button', { hasText: 'فهمیدم، بریم طوفان' }).first();
    if (await mayhemIntro.count()) {
      await mayhemIntro.click({ timeout: 5000 }).catch(() => {});
