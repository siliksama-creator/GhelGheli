...
//
// استثنای «دوئل طوفان»: در راند دو‌امتیازی یک برچسب/قاعدهٔ شرطیِ اضافه
// می‌شود که فقط در حالت طوفان رندر می‌شود (isStorm). این متن هم در وب و
// هم اندروید دقیقاً یکی است و با رنگ/آیکون نه با جمله‌های پراکنده.
check('بازطراحی هیچ متنِ تازهٔ بی‌قیدی به صحنه اضافه نکرده', () => {
  ...
  // متن‌های پایه: شمارهٔ راند، برچسب، نامِ معیار، قاعده.
  const slots = (block.match(/<(small|label|b|em)>/g) || []).length;
  // حداکثر یک em شرطیِ طوفان (زیرنویس «اگر مساوی شد…») اضافه است.
  assert(slots >= 4 && slots <= 5,
    `صحنه باید ۴ خانهٔ متنی (۵ در حالت طوفان) داشته باشد، ${slots} تا دارد`);
  // هر متنِ اضافه باید صریحاً به isStorm گره خورده باشد.
  const stormConditions = (block.match(/isStorm/g) || []).length;
  if (slots === 5) {
    assert(stormConditions >= 2, 'متنِ پنجم باید مشروط به isStorm باشد');
  }
});

// ── ۹٫۵. آینگیِ طوفان بین وب و اندروید ──────────────────────────────────────
check('اعلانِ راند دو‌امتیازی در هر دو پلتفرم هست و ۲/وقت‌اضافه می‌گوید', () => {
  assert(jsx.includes('isStorm') && jsx.includes('دو‌امتیازی'),
    'وب حالت طوفان و برچسب دو‌امتیازی را ندارد');
  assert(android.includes("mod == 'storm'") || android.includes("widget.mod == 'storm'")
    || android.includes("mod:") && android.includes('راند دو‌امتیازی'),
    'اندروید حالت طوفان را ندارد');
  assert(android.includes('راند دو‌امتیازی') && android.includes('وقت اضافه'),
    'اندروید برچسب دو‌امتیازی/وقت اضافه را ندارد');
  assert(jsx.includes('وقت اضافه'), 'وب وقت اضافه را ندارد');
  // رنگ آتشیِ مشترک.
  assert(jsx.includes('#FF7A1A') && android.includes('0xFFFF7A1A'),
    'رنگ آتشیِ طوفان باید در هر دو سمت یکسان (FF7A1A) باشد');
});
