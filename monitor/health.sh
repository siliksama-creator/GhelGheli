...
for PORT in $PORTS; do
  ...
  if probe_node "$PORT" > /tmp/.gg-probe 2>/dev/null; then
    ...
    NODES_OK=$((NODES_OK + 1))
    # ⚠️ باگِ واقعیِ ۲۹ شهریور (دوّمین لایهٔ همان هشدارِ نیمه‌شب): حالتِ آلارمِ
    # گره فقط «روشن» می‌شد و هیچ‌وقت «پاک» نمی‌شد — پرچمِ ALARM_node_XXXX در
    # فایلِ وضعیت برای همیشه ۱ می‌ماند. نتیجه‌اش دو چیز بود:
    #   ۱) پیامِ «✅ برطرف شد» برای گره‌ها هرگز نمی‌آمد؛
    #   ۲) بدتر: افتِ **بعدیِ** همان گره بی‌صدا می‌ماند (alarm() که پرچمِ ۱ را
    #      می‌دید، «قبلاً خبر داده‌ام» حساب می‌کرد و چیزی نمی‌فرستاد).
    # حالا با سالم‌بودنِ گره، پرچمش پاک می‌شود؛ دقیقاً مثلِ دیسک و رم.
    alarm "node_${PORT}" "0" ""
  else
    BODY="$(cat /tmp/.gg-probe)" 
