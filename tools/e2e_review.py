...
def noise(seed):
    ...
    ⚠️ نسخهٔ اول مربع‌های رنگیِ تصادفی می‌کشید و **تأیید خودکار
       می‌گرفت**. اول شبیهِ باگِ موتور به نظر رسید، ولی اندازه‌گیریِ
       محلی نشان داد نمره‌اش ۰.۲۵۰ است در برابرِ آستانهٔ ۰.۲۰ کدِ
       نام‌دار — یعنی موتور درست عمل کرده و **دادهٔ تست** بد بوده:
       آن مربع‌ها به‌قدرِ کافی لبه و بافت داشتند که شبیهِ کارت شوند.

       آستانهٔ ۰.۲۰ عمداً پایین است (خواستهٔ مالک): وقتی کد از پیش به
       کارت گره خورده، عکس فقط باید ثابت کند کاربر کارتِ فیزیکی را در
       دست دارد، نه اینکه هویتش را تعیین کند.

       پایین آوردنِ آستانه برای سبز شدنِ تست، همان اشتباهی بود که چند
       بار در این پروژه تکرار شده. پس تصویر عوض شد:

           تصویرِ تخت    ۰.۰۳۲
           تصویرِ تاریک  ۰.۰۳۹
           دیوارِ سفید   ۰.۰۰۰   ← این
           مربع‌های رنگی ۰.۲۵۰   ← نسخهٔ قبلی

       یک سطحِ تقریباً یکنواخت با نویزِ بسیار ملایم — چیزی که واقعاً
       «عکسِ دیوار» است، نه کارت.
    """
    base = 236 - (seed % 3) * 6
    im = Image.new('RGB', (400, 560), (base, base, base - 4))
    d = ImageDraw.Draw(im)
    # چند سایهٔ خیلی کم‌رنگ تا فایل JPEG کاملاً یکدست نباشد (فشرده‌سازیِ
    # تصویرِ تک‌رنگ گاهی ابعادِ عجیب می‌دهد) ولی هیچ لبهٔ واقعی نسازد.
    for i in range(6):
        v = base - 3 - i
        d.ellipse([40 + i * 30, 60 + i * 40, 300 + i * 12, 380 + i * 20],
                  outline=(v, v, v - 3), width=1)
    o = io.BytesIO()
    im.save(o, 'JPEG', quality=72)
    return o.getvalue()
