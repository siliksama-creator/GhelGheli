...
# ⚠️ معیار عوض شد و دلیلش مهم است.
#
# نسخهٔ قبلی انتظار داشت دقیقاً **یک پروندهٔ pending** ساخته شود. آن
# وقتی درست بود که عکسِ تار همیشه به صف بررسی می‌رفت.
#
# حالا آستانهٔ کدِ بی‌نام ۴۰٪ است و همین عکس مستقیم `approved` می‌شود،
# پس هیچ ردیفِ `pending` ای نمی‌ماند. شمردنِ pending دیگر چیزی را
# نمی‌سنجد.
#
# چیزی که واقعاً اهمیت دارد و باید تضمین شود: از چهار درخواستِ هم‌زمان
# با **یک عکس**، دقیقاً یکی باید موفق شود. بقیه هر کدام یا رد شوند یا
# تکراری اعلام شوند — ولی در هیچ حالتی کاربر نباید چهار کارت بگیرد.
granted = pend + sum(1 for _c, _s, r in res if r.get('status') == 'approved')
print(f'\nکارتِ داده‌شده: {granted} (ایده‌آل ۱)')
print(f'ردشده به‌عنوان تکراری: {dup}')
print(f'کدهای رزروشده: {len(resv)} → {[c["code"] for c in resv]}')
if granted == 1:
    print('✓ گارد حتی زیر فشار هم‌زمانی هم گرفت — فقط یک کارت داده شد')
else:
    print(f'⚠ {granted} کارت داده شد به‌جای ۱ — یک عکس چند کارت گرفت')
    sys.exit(1)
