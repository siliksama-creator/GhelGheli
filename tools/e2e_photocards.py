...
⚠️⚠️ این فایل **کهنه است و اجرا نشود** — با `e2e_photospec.py` و
      `e2e_photov2.py` جایگزین شده. توضیح پایین.
...
اجرا شد و چهار مورد قرمز داد. هر چهار مورد **خطای خودِ تست** بودند نه
باگِ محصول — و همین خطرناکشان می‌کند: تستی که مرتب قرمزِ بی‌دلیل بدهد،
خیلی زود کلاً نادیده گرفته می‌شود و آن روز که قرمزِ واقعی بدهد هم کسی
نگاهش نمی‌کند.

  ۱. «تولید ۶۰ کد» و «آمار کدها» → ۴۰۴
     مسیرِ `/admin/photo-cards/codes/generate` **عمداً حذف شده**. بانکِ
     کدِ مشترک با «کدهای گره‌خورده به کارت» جایگزین شد (مایگریشن ۰۴۱)،
     چون کدِ بی‌نام یعنی هر کسی که کد را بداند هر کارتی را ادعا کند.
     هیچ کلاینتی دیگر صدایش نمی‌زند — بررسی شد.

  ۲. «طرح تکراری رد می‌شود» → ۲۰۰ به‌جای ۴۰۹
     این یکی جدی به نظر می‌رسید، پس **مستقیم آزموده شد**: همان فایلِ
     PNG با نامِ دیگر آپلود شد و سرور ۴۰۹ داد با
     `similarity = 1.0` و پیامِ درست. یعنی گارد سالم است.

     علتِ قرمزیِ تست: تصویرِ گرادیانیِ ساختگی‌اش بعد از بهبودهای موتور
     (rgb_sig، text_tokens، برشِ خودکار) دیگر با خودش ۹۳٪ شباهت
     نمی‌گیرد. یعنی تست دادهٔ نامناسب می‌فرستد، نه اینکه سرور اشتباه
     کند.

  ۳. آرگومان‌هایش با بقیهٔ ابزارها هم‌خوان نیست و کاربر را از قبل
     می‌خواهد، در حالی که بقیه خودشان می‌سازند.

جانشین‌ها که همین حوزه را با تصاویرِ واقعی پوشش می‌دهند:
    tools/e2e_photospec.py    ۳۰ بررسی
    tools/e2e_photov2.py      ۱۷ بررسی
    tools/e2e_twoside.py      رو و پشت با کارتِ واقعی
    tools/e2e_bound.py        کدهای گره‌خورده
"""
...
if '--force' not in _sys_guard.argv:
    raise SystemExit(
        ...
        '    مسیرِ «بانکِ کدِ مشترک» که می‌سنجد عمداً حذف شده است.\n'
        '    جایگزین‌ها: e2e_photospec.py · e2e_photov2.py · e2e_twoside.py\n'
