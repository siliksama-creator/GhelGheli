...
# ═══════════════════════════════════════════════════════════════════════════
# پیشوندِ یکتا برای هر اجرا — چرا لازم است
# ═══════════════════════════════════════════════════════════════════════════
#
# قبلاً کدها ثابت بودند (SPEC-0001 …). بانکِ کدِ «کارت با عکس» عمداً
# مشترک و پایدار است، پس اجرای دومِ تست همان کدها را می‌دید در حالی که
# اجرای اول آن‌ها را `used` کرده بود → «این کد قبلاً استفاده شده است».
#
# این ایرادِ سرور نیست؛ سرور دقیقاً درست رفتار می‌کند. ایرادِ تستی است
# که فرض می‌کرد دیتابیس هر بار خالی است.
#
# با پیشوندِ یکتا هر اجرا فضای نامِ خودش را دارد و تست چند بار پشت سر
# هم قابل اجراست — که برای باگ‌گیری حیاتی است.
PFX=f'SP{int(time.time())%100000:05d}'
# توکن از کش می‌آید تا سقفِ ۱۰ ورود در ۱۵ دقیقه نسوزد — توضیح در _authcache.py
atok=admin_token(apw)

# ═══════════════════════════════════════════════════════════════════════════
# کاربرِ تازه به‌ازای هر اجرا — چرا حیاتی است
# ═══════════════════════════════════════════════════════════════════════════
#
# این تست عمداً «۵ کد غلط پشت سر هم» را می‌آزماید تا قفلِ ۳ ساعته را
# بسنجد، و در مجموع بیش از ۲۰ بار /photo-cards/submit را صدا می‌زند.
#
# حالا که محدودکنندهٔ نرخ درست شده و روی **کاربر** کلید می‌خورد (قبلاً
# روی IP بود و اشتباهاً همه را با هم می‌شمرد)، اجرای دوم با همان کاربر
# به سقفِ ۲۰ در ساعت می‌خورد و همهٔ گام‌ها ۴۲۹ می‌گیرند.
#
# ⚠️ نکتهٔ مهم: این یعنی محدودکننده **درست کار می‌کند**. وسوسه‌کننده
#    است که سقف را بالا ببریم تا تست سبز شود — که یعنی خرابکاری در
#    محصول برای راحتیِ تست. راه درست این است که تست هر بار کاربرِ
#    تازه بسازد، همان‌طور که کاربرِ واقعی سهمیهٔ خودش را دارد.
#
# قفلِ ۳ ساعته هم به همین شکل حل می‌شود: کاربرِ تازه شمارندهٔ صفر دارد،
# پس دیگر لازم نیست بین اجراها جدولِ photo_card_attempts دستی خالی شود.
...
if st==200 and ru.get('token'):
    ...
    # با باگِ واقعی اشتباه نگیرد.
    print(f'  ⚠ ساخت کاربر تازه نشد ({st}) — کاربرِ ثابت استفاده می‌شود؛'
          ' ممکن است به سقفِ نرخ بخورد')
    _,u=req('POST','/api/auth/login',body={'mobile':'09001112233','password':'Qa!12345'})
    utok=u['token']

# ═══════════════════════════════════════════════════════════════════════════
# پاکسازیِ اجرای قبلی — چرا لازم شد
# ═══════════════════════════════════════════════════════════════════════════
#
# این تست طرح‌هایی با نامِ SPEC-* می‌سازد و آن‌ها در دیتابیس می‌مانند.
# تا وقتی محافظِ «طرح تکراری» خراب بود (۵۰۰ می‌داد به‌جای ۴۰۹) کسی
# متوجه نشد؛ حالا که درست کار می‌کند، اجرای دومِ تست بلافاصله ۴۰۹
# می‌گیرد چون طرحِ اجرای قبلی هنوز فعال است.
#
# این ایرادِ محافظ نیست — دقیقاً همان کاری را می‌کند که باید. ایرادِ
# تست است که بعد از خودش تمیز نمی‌کرد.
#
# طرح‌ها **غیرفعال** می‌شوند و حذف نمی‌شوند: ممکن است پرونده یا
...
stA,rA=req('POST','/api/admin/photo-cards/designs',atok,{'name':f'{PFX}-آبی','pointValue':'120'},{'image':('a.png',pngA,'image/png')})
...
codes=[f'{PFX}-{i:04d}' for i in range(1,16)]
...
sample=[c for c in r.get('codes',[]) if str(c['code']).startswith(PFX)]
...
st,r=req('POST','/api/photo-cards/submit',utok,{'code':f'{PFX}-0001'},{'image':('s.jpg',shot(imA),'image/jpeg')})
ck('پذیرفته شد',st==200 and r.get('status')=='approved',f'{st} {r}')
ck('کارتِ درست تشخیص داده شد',r.get('cardType')==f'{PFX}-آبی',str(r.get('cardType')))
...
row=[c for c in r2.get('codes',[]) if str(c['code'])==f'{PFX}-0001']
ck('کد قفل شد (used)',row and row[0]['status']=='used',str(row[0]['status'] if row else '—'))
ck('کد حالا به همان طرح گره خورد',row and row[0].get('card_type_name')==f'{PFX}-آبی',str(row[0].get('card_type_name') if row else '—'))

print('\n══ گام ۴: عکسِ کارتِ دوم با کدِ دیگر → کارتِ دوم، نه اولی ══')
st,r=req('POST','/api/photo-cards/submit',utok,{'code':f'{PFX}-0002'},{'image':('s.jpg',shot(imB),'image/jpeg')})
ck('کارتِ دوم درست تشخیص داده شد',st==200 and r.get('cardType')==f'{PFX}-نارنجی',f'{st} {r.get("cardType")}')
...
st,r=req('POST','/api/photo-cards/submit',utok,{'code':f'{PFX}-0003'},{'image':('n.jpg',nb.getvalue(),'image/jpeg')})
...
subs=[s for s in r.get('submissions',[]) if s.get('code')==f'{PFX}-0003']
...
st,r=req('POST','/api/photo-cards/submit',utok,{'code':f'{PFX}-0004'},{'image':('s.jpg',shot(imA),'image/jpeg')})
...
st2,r2=req('GET',f'/api/admin/photo-cards/codes?q={PFX}-0004',atok)
