...
async def main():
    ...
    try:
        async with async_playwright() as p:
            ...
            await pg.locator('.ptSearchRow button').first.click()
            ...
            # ⚠️ به‌جای تطابقِ کاملِ رشته، بخشِ یکتا. متنِ رندرشده ممکن
            #    است فاصله یا نویسهٔ نامرئیِ متفاوت داشته باشد و تطابقِ
            #    دقیق، قرمزِ دروغین می‌دهد.
            ck('دلیل در جدول دیده می‌شود', 'کسر آزمایشی' in body,
               'دلیل ثبت نشد')
            ...
            ck('جدولِ برترین‌ها رندر شد',
               await pg.locator('table').count() >= 1)
            # ⚠️ نسخهٔ اول دنبالِ شمارهٔ همین کاربر می‌گشت و گاهی قرمز
            #    می‌شد — نه به‌خاطر باگ، بلکه چون فهرست فقط ۵۰ نفرِ برتر
            #    را نشان می‌دهد و کاربرِ تازه با ۶۰۰ امتیاز ممکن است
            #    پایین‌تر بیفتد. معیارِ درست این است که جدول **داده**
            #    داشته باشد و ستون‌هایش پر باشند.
            rows_n = await pg.locator('table tbody tr').count()
            ck(f'جدول ردیفِ واقعی دارد ({rows_n})', rows_n >= 1,
               'جدولِ برترین‌ها خالی است')
            # ⚠️ نیم‌فاصله (U+200C) در «یک‌باره» — نسخهٔ اولِ این بررسی
            #    فاصلهٔ معمولی نوشته بود و قرمزِ دروغین می‌داد. برای
            #    مقایسهٔ متنِ فارسی، نیم‌فاصله باید نرمال شود.
            norm = body.replace('\u200c', ' ')
            ck('ستونِ «بیشترین یک‌باره» پر است',
               'بیشترین یک باره' in norm, 'ستون رندر نشد')
