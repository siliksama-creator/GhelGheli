...
# ═══════════════════════════════════════════════════════════════════════════
head "۴) موجِ حمله — قراردادِ سکوت (مهر ۱۴۰۵): لاگ + شمارش، نه پیام"
# ═══════════════════════════════════════════════════════════════════════════
# مالک خواست موج‌هایی که fail2ban خودش مهار کرده پیامِ جداگانه نگیرند؛
# پس این بخش حالا «نیامدنِ پیام» و «حضورِ کامل در لاگ» را می‌سنجد.
for i in 1 2 3; do ban $((NOW - 100 * i)) sshd "203.0.113.$i"; done
for i in 1 2 3; do ban $((NOW - 60 * i)) nginx-scan "198.51.100.$i"; done
OUT="$(run_dry)"
printf '%s' "$OUT" | grep -q 'موجِ حملهٔ خودکار به سرور' \
  && bad "موج پیامِ جداگانه گرفت (قراردادِ سکوت شکست!)" || ok "موج پیام نگرفت (قراردادِ سکوت)"
grep -qF 'SUPPRESSED (آزمایشی) [campaign_sshd]' "$ROOT/sshwatch.log" \
  && ok "موجِ SSH با برچسبِ SUPPRESSED و نامِ درستِ جیل لاگ شد" || bad "موجِ SSH ساکتِ لاگ نشد"
grep -qF 'SUPPRESSED (آزمایشی) [campaign_nginx-scan]' "$ROOT/sshwatch.log" \
  && ok "موجِ اسکنِ وب هم SUPPRESSED لاگ شد" || bad "موجِ اسکنِ وب لاگ نشد"
grep -q '203.0.113.1' "$ROOT/sshwatch.log" && ok "آی‌پی‌های موج در لاگ ماندند (چیزی گم نشد)" || bad "آی‌پی‌ها در لاگ نبودند"
# ضدِ اسپم و شمارنده فقط در حالتِ واقعی معنا دارد (dry-run مُهر/شمارش نمی‌گذارد)
run_real >/dev/null; run_real >/dev/null
SC="$(grep -c "ALERT \[campaign_sshd\]" "$ROOT/sshwatch.log" || true)"
[ "$SC" = "0" ] && ok "موج هرگز ALERT نگرفت (قراردادِ تازه)" || bad "موج ALERT گرفت: ${SC}×"
[ "$(cat "$ROOT/state/suppressed.count" 2>/dev/null || echo 0)" = "2" ] \
  && ok "شمارنده هر موج را یک بار شمرد (نه هر اجرا)" \
  || bad "شمارنده غلط: $(cat "$ROOT/state/suppressed.count" 2>/dev/null || echo -)"
...
OUT="$(run_dry)"
printf '%s' "$OUT" | grep -q 'تلاشِ پیوستهٔ رمزِ SSH' \
  && bad "تلاشِ پیوسته پیام گرفت (قراردادِ سکوت شکست!)" || ok "تلاشِ پیوسته پیامِ جداگانه نگرفت (سکوت)"
grep -qF 'SUPPRESSED (آزمایشی) [persist_203.0.113.250]' "$ROOT/sshwatch.log" \
  && ok "تلاشِ پیوستهٔ «کند» تشخیص و SUPPRESSED لاگ شد" || bad "تلاشِ پیوسته لاگ نشد"
