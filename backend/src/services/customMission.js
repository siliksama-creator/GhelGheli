 ...
 *
 * خواستهٔ مالک (۲۹ شهریور): «باید تا ۱۰ تا ماموریت هم اگه بخوام ادد کنم یا
 * حذف کنم یا ویرایش کنم.» پس سقف از ۵ به ۱۰ رفت. سقف لازم است چون هر
 * ماموریت یک کارتِ تمام‌عرض در بالای «ماموریت‌های امروز» است؛ بدونِ آن،
 * ادمینِ خوش‌ذوق می‌تواند صفحهٔ کاربر را با ۳۰ کارت پر کند و خودِ
 * ماموریت‌های روزانه از دید بیرون برود.
 *
 * ⚠️ عدد **فقط این‌جا** است: پنل آن را از `max` پاسخِ API می‌خواند و هر دو
 *    کلاینت فهرست را حلقه می‌زنند، پس جای دیگری سقفِ سخت‌کد وجود ندارد.
 */
const MAX_ITEMS = 10;
...
/** مُهرِ زمانیِ فهرست — پنل با آن جلوی ذخیرهٔ پنلِ کهنه را می‌گیرد. */
function listStamp() {
  try {
    return normalizeStored(opsConfig.syncGet(KEY)).updatedAt || null;
  } catch {
    return null;
  }
}
...
async function saveMany(adminId, payload) {
  ...
  // ── گاردِ «فهرستِ کهنه» ────────────────────────────────────────────────
  // پنل لحظهٔ بارشدن، مُهرِ زمانیِ فهرست را می‌گیرد و هنگامِ ذخیره پس
  // می‌فرستد. اگر در این فاصله پنل/تبِ دیگری فهرست را عوض کرده باشد، ذخیره
  // ۴۰۹ می‌گیرد به‌جای اینکه بی‌صدا کارِ آن یکی را پاک کند.
  //
  // چرا لازم است: با چندپروسه‌شدنِ سرور، ادمین می‌توانست تبِ قدیمی را ذخیره
  // کند و فهرستِ تازه (مثلاً ۱۰ ماموریت) به فهرستِ لحظهٔ بارشدنِ آن تب
  // (مثلاً ۱ ماموریت) برگردد — دقیقاً همان «فهرست دوباره برمی‌گرده رو یکی».
  // اگر پنلِ قدیمی این فیلد را نفرستد، گارد بی‌صدا غیرفعال است (سازگاری).
  const since = payload?.ifUnchangedSince;
  if (since) {
    const storedStamp = normalizeStored(opsConfig.syncGet(KEY)).updatedAt;
    if (String(storedStamp || '') !== String(since)) {
      const e = new Error('فهرست از لحظه‌ای که این صفحه باز شد عوض شده — '
        + 'صفحه را دوباره بخوان و بعد ذخیره کن (تا کارِ پنلِ دیگر پاک نشود)');
      e.status = 409;
      e.code = 'stale_list';
      throw e;
    }
...
module.exports = {
  ...
  list, listStamp, activeItems, current, publicView, status, statuses,
