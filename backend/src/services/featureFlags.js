...
const DEFAULTS = Object.freeze({
  ...
  // ═══════════════════════════════════════════════════════════════════════
  //
  // تیکِ صفحهٔ پنل. `true` یعنی بخش می‌تواند دیده شود؛ ولی نمایشِ واقعیِ
  // ردیف در منو و صفحه، مشروط به این هم هست که **حداقل یک برنامهٔ فعال**
  // وجود داشته باشد. پس روشن‌بودنِ پیش‌فرض هیچ چیزی را به محصول اضافه
  // نمی‌کند تا ادمین اولین برنامه را ثبت کند.
  //
  // ⚠️ چرا نه `false`: تیکِ خاموش یعنی «اگر محتوا هم بود پنهانش کن».
  //    با پیش‌فرضِ خاموش، ادمینی که برنامه ثبت می‌کند و تیک را نمی‌بیند
  //    فکر می‌کند قابلیت خراب است — دقیقاً همان تجربهٔ «پنل کار نمی‌کند»
  //    که این پروژه چند بار از آن ضربه خورده.
...
function normalizeFeatures(raw) {
  ...
  return {
    ...
    games,
    wheel: v.wheel !== false,
    // خاموشی فقط با `false` صریح. نبودنِ کلید = روشن (توضیح بالا).
    recommendedApps: v.recommendedApps !== false,
  };
}
