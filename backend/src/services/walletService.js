...
const VALID_SOURCES = new Set([
  ...
  'pass',
  // شارژ کیف پول از درگاه (کافه‌بازار) و بازگشت وجه آن.
  //
  // ⚠️ این فهرست باید همیشه با CHECK جدول `wallet_transactions` در
  // دیتابیس یکی بماند. اگر منبعی فقط به یکی از این دو اضافه شود، خطا
  // در «زمان اجرا» ظاهر می‌شود نه در تست: یا اینجا رد می‌شود، یا
  // پایگاه‌داده تراکنش را برمی‌گرداند و پولِ پرداخت‌شده شارژ نمی‌شود.
  // (مایگریشن متناظر: 067_payment_orders.sql)
  'topup', 'topup_refund',
]);
