...
const VALID_SOURCES = new Set([
  ...
  // مثل 'shop' کمیسیونِ معرف نمی‌دهد — اجرایش در shopService است.
  'card_box',
  // خریدِ اشتراکِ پلاس با موجودیِ کیف پول (خواستهٔ مالک، ۱۴۰۵/۰۷/۰۲).
  // در CHECK دیتابیس از قبل مجاز بود (مهاجرتِ ۰۶۷/۰۷۴) و فقط اینجا غایب
  // بود — دقیقاً همان وضعیتی که 'shop' قبلاً داشت.
  //
  // ⛔ مثل 'shop' و 'card_box' کمیسیونِ معرف نمی‌دهد: سهمی که از کیف پول
  //    پرداخت می‌شود پولِ تازه وارد سیستم نمی‌کند، پس چیزی برای تقسیم
  //    وجود ندارد. اجرایش در `shopService.deliverPlus` است.
  'subscription',
  // جایزهٔ لیگِ معرف‌ها (۴ مهر ۱۴۰۵). مثلِ 'league'، منبعِ جدا لازم است:
  // گزارشِ مالی باید سهمِ کمپین‌های دعوت را از تنظیمِ دستیِ مدیر و از
  // لیگِ ماهانه تفکیک کند. CHECK متناظر در مایگریشن ۱۰۱.
  'invite_league',
]);

// منابعِ تاریخی — برای خوانایی گزارش‌ها و فیلترِ پنل ادمین.
//
// از CHECK دیتابیس حذف **نشده‌اند** چون ردیف‌های قدیمی باید معتبر بمانند.
//
// ⚠️ «تاریخی» لزوماً یعنی «مرده» نیست: `shop` (دورِ ۲۲) و `subscription`
//    (۱۴۰۵/۰۷/۰۲) هر دو دوباره زنده شدند و در `VALID_SOURCES` هم هستند.
//    اینجا می‌مانند چون ردیف‌های دورهٔ قبلی‌شان هنوز در دفترکل‌اند و
//    فیلترِ پنل باید بشناسدشان. فقط `topup*` واقعاً مرده است.
const LEGACY_SOURCES = Object.freeze([
  // خرید شاپ و پلاس از موجودی کیف پول — دوباره فعال (بالا را ببینید).
  'shop', 'subscription',
