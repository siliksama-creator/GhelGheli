...
const RULE_DEFS = Object.freeze({
  ...
  rewardSeconds: {
    value: 4, min: 2, max: 8,
    label: 'ثانیهٔ نمایشِ «لحظهٔ جایزه»',
    hint: 'هر بار که کاربر امتیاز/سکه/آیتمی می‌گیرد یا نتیجهٔ بازی را می‌بیند، این کارت جلوی چشمش می‌آید. کوتاه‌ترش یعنی زودتر می‌رود؛ کارت هیچ‌وقت جلوی ادامهٔ کار را نمی‌گیرد.',
  },
...
const DEFAULT_COPY = Object.freeze({
  ...
  plus: {
    ...
    benefitsNote: 'دسترسی قاب‌ها و افکت نام، ستاره پلاس و Premium Pass برای {days} روز فعال می‌شود.',
  },
  ...
  reward: {
    ...
    memory: 'نبرد جفت‌یاب',
    league: 'جایزهٔ لیگ',
    passAll: '{count} جایزهٔ گذر نبرد',
    // ── نتیجهٔ بازی ──
    // خواستهٔ مالک (۲۹ شهریور): «زمانی که بازی رو می‌بازه نمی‌دونم چطوری باید
    // بگم که اعصابش خورد نشه.» تصمیمِ محصولی: هیچ‌جا کلمهٔ «باخت» نمی‌آید،
    // رنگِ قرمز و آیکونِ غمگین هم نه. جمله روی «این دور تمام شد» و
    // «چیزی که از دست ندادی» می‌نشیند (سکه هرگز کم نمی‌شود). لحن از پنل
    // قابلِ تغییر است، پس اگر نپسندیدید بدونِ آپدیت عوض می‌شود.
    winTitle: 'بردِ تو ثبت شد',
    winLine: 'حسابِ این برد در پروفایلت ثبت شد',
    drawTitle: 'پایاپای',
    drawLine: 'هیچ‌کس کم نیاورد؛ یک دستِ دیگر؟',
    lossTitle: 'این دور تمام شد',
    lossLine: 'سکه‌هایت سرِ جایشان است — چیزی از دست ندادی',
  ...
  // راهنمای ثبت کارت — خواستهٔ مالک ۲۳ شهریور: دو خطِ ثابت روی تب،
  // قابل‌ویرایش از پنل تا بدون APK عوض شود.
  cardReg: {
    lead: 'از کارت عکس بگیرید و کد را وارد کنید',
    minPointsNote: 'فقط کارت‌های ۵۰۰ امتیازی و بالاتر ثبت می‌شود.',
    // ── چرا این جمله زنده است و نه ثابت در کد ────────────────────────
    // خواستهٔ مالک (۳ مهر ۱۴۰۵): «این جمله را با احترام اضافه کن» — یک
    // توضیحِ محصولی که لحنش مهم است، چون دارد به کاربر می‌گوید «ممکن است
    // فلان بازیکنِ افسانه‌ای از یک تازه‌وارد کمتر اثر بگذارد». هر بار که
    // کسی از این جمله آزرده شود یا لحنش را نپسندند، باید بدونِ آپدیتِ
    // اپ عوض شود؛ پس جای درستش همین `live_copy` است، نه رشتهٔ سفت.
    duelEffectNote: `کارت های قلقلی براساس قدرت بازیکن و درصد کمیاب بودن در بازی Duel card تاثیر میذارن این به این معنیه که ممکنه بازیکن افسانه ای مثل پله از بازیکن جدیدی بخاطر اینکه سبک کارتش کمیاب نبوده و افکت اصلی کارتش ضعیف تر هستش دست رو ببازه با احترام به تمامی بازیکن ها قدیمی و افسانه ای سیستم به این صورت عمل میکنه.`,
    // ── جملهٔ دومِ همین کادر (خواستهٔ مالک، ۳ مهر ۱۴۰۵) ──────────
    // «کارت‌های خاص نقره‌ای/طلایی/پلاتینیوم و غیره فعلاً در اپلیکیشن
    // ثبت نمیشن و پشتیبانی روبیکا این کارت‌ها رو ثبت می‌کنه.»
    //
    // ⚠️ کلیدِ **جدا** و نه دنبالهٔ جملهٔ بالایی: دو موضوعِ متفاوت‌اند
    // (یکی اثرِ کلاسِ کارت در دوئل، دیگری این‌که چه کارتی از اپ ثبت
    // می‌شود) و ادمین باید بتواند یکی را بی‌دست‌زدنِ دیگری عوض کند.
    // روزی که خودِ اپ این کارت‌ها را بپذیرد، همین یک کلید خالی/عوض
    // می‌شود — بدونِ آپدیتِ اپ. همان دلیلِ زنده‌بودنِ جملهٔ بالایی.
    specialCardsNote: `کارت های خاص نقره ای طلایی پلاتینیوم و غیره فعلا در اپلیکیشن ثبت نمیشن و پشتیبانی روبیکا این کارت هارو ثبت میکنه`,
  },
  // ── لیگِ معرف‌ها (۴ مهر ۱۴۰۵) ─────────────────────────────────────────
  //
  // خواستهٔ مالک: تبِ تازهٔ «تاپ دعوت‌کنندگان» + «لیگ معرف‌ها». اسم‌ها و
  // جمله‌های حالتِ خالی این‌جا زنده‌اند چون لحنشان محصولی است و باید بدونِ
  // آپدیتِ اپ عوض شوند (همان دلیلی که `referral.dailySpinRule` زنده شد).
  inviteLeague: {
    allTimeTitle: 'معرف‌های برتر تا کنون',
    leagueTitle: 'لیگ معرف‌ها',
    // حالتِ خالی — تصمیمِ مالک: «اگه ادمین لیگ معرف راه ننداخت باید فعلاً
    // لیگ ای برای معرف ها ساخته ننشده».
    emptyNote: 'هنوز لیگی برای معرف‌ها ساخته نشده است. هر وقت مدیر یک لیگ بسازد، همین‌جا برگزار می‌شود.',
    rulesNote: 'فقط دعوتِ معتبر شمرده می‌شود (کاربری که با کد شما ثبت‌نام کرده و فعال است). اگر دو نفر تعدادشان برابر شود، آن‌که زودتر به آن عدد رسیده بالاتر می‌ایستد.',
    payoutNote: 'جایزه پس از پایانِ لیگ، بعد از تأییدِ مدیر به حسابت واریز می‌شود.',
    myRankNote: 'رتبهٔ تو همیشه پایینِ جدول می‌آید؛ حتی وقتی بیرونِ ده نفرِ اول باشی.',
  },
  wheel: {
  ...
  update: {
    ...
    reload: 'تازه‌سازی',
    later: 'بعداً',
    // ── دیالوگِ دانلودِ خودکارِ اندروید (مهر ۱۴۰۵) ────────────────────
    // `action` (دکمهٔ «به‌روزرسانیِ» دیروز که مرورگر باز می‌کرد) با آمدنِ
    // دانلودِ داخل‌اپی از هر دو کلاینت برداشته شد، پس از سرور هم رفت —
    // وگرنه گاردِ «کلیدِ بی‌مصرف» قرمز می‌شد. نسخه‌های قدیمیِ اپ که هنوز
    // `update.action` را می‌خوانند، به فول‌بکِ خودشان («به‌روزرسانی»)
    // می‌رسند که واژه‌به‌واژه همین بود — پس هیچ‌چیز نمی‌شکند.
    downloading: 'در حال دانلود نسخهٔ تازه…',
    verifying: 'در حال بررسی فایل دانلودشده…',
    installing: 'در حال باز کردن نصب‌کننده…',
    install: 'نصب',
    retry: 'تلاش مجدد',
    browser: 'دانلود با مرورگر',
    error: 'دانلود ناموفق بود. اتصال را بررسی کن و دوباره تلاش کن.',
    forcedNote: 'برای ادامهٔ کار با قلقلی، نصب نسخهٔ تازه لازم است.',
  },
});
...
const COPY_CONTRACT = Object.freeze({
  ...
  cardReg: {
    lead: [], minPointsNote: [],
    // بدونِ جای‌نگهدار: جمله هیچ عددی ندارد؛ اگر روزی «۳٬۰۰۰ امتیاز» به آن
    // اضافه شد، عدد باید از `live_rules` بیاید نه از متنِ سفت.
    duelEffectNote: [],
    // بدونِ جای‌نگهدار: جمله هیچ عددی ندارد — شرایطِ ثبت را
    // پشتیبانی تعیین می‌کند، نه یک عدد در متنِ کاربر.
    specialCardsNote: [],
  },
  wheel: {
    resetNote: [],
  ...
  inviteLeague: {
    allTimeTitle: [], leagueTitle: [], emptyNote: [],
    rulesNote: [], payoutNote: [], myRankNote: [],
  },
  games: {
  ...
  update: {
    ...
    reload: [],
    later: [],
    // کلیدهای دیالوگِ دانلودِ خودکار — هیچ‌کدام جای‌نگهدار ندارند؛
    // درصد و حجم با `faNum` در خودِ ویجت ساخته می‌شوند، نه در قالب.
    downloading: [],
    verifying: [],
    installing: [],
    install: [],
