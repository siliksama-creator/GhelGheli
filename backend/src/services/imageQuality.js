...
//
// ⚠️ کالیبراسیون با دادهٔ واقعی (مهر ۱۴۰۵): سنجهٔ تاری «واریانسِ لاپلاسین،
// تقسیم بر ۱۰۰» است. اندازه‌گیری روی ۱۳۱ تصویر مرجعِ تمیزِ کارت نشان داد:
//   • عکسِ واضحِ کارت  → میانهٔ ~۳۱، بازهٔ ۱۴ تا ۴۰ (گرافیکِ نرمِ کارت ذاتاً
//     لبه‌های پرتکرارِ متنِ سند را ندارد، پس عددش پایین است — این تار نیست).
//   • تاریِ فاجعه‌بار (شبیه‌سازیِ لرزش دست، radius 4) → میانهٔ ~۴.۵، بازهٔ ۱–۶.
//   • تاریِ خفیف (radius 2) → میانهٔ ~۱۶.
// آستانهٔ قبلی ۶۰ بود که حتی از واضح‌ترین کارت هم بالاتر بود و عملاً **همهٔ**
// عکس‌ها را رد می‌کرد (باگِ «هر عکسی تار است» در تستِ اپ). آستانهٔ درست باید
// وسطِ فاصلهٔ خالیِ بینِ «تاری خفیف (~۱۶)» و «فاجعه (~۴.۵)» بنشیند: ۸ یعنی
// فقط تصویرِ واقعاً بی‌جزئیات برگردانده می‌شود؛ عکسِ متوسط که موتور می‌تواند
// بخواند رد نمی‌شود (آن پرونده در صورتِ تردید به صفِ مدیر می‌رود، نه رد).
const BLUR_MIN = 8;         // واریانسِ لاپلاسین زیر این = تارِ فاجعه‌بار
