...
/**
 * منابعِ امتیازی که کمیسیون می‌سازند — **عمداً خالی**.
 *
 * ⚠️ تغییرِ دامنه به‌خواستِ مالک:
 *
 *   «کمیسیون ۵٪ دعوت فقط از خرید کامل‌شدهٔ شاپ باشد، نه از امتیاز
 *    بازی/تپ.»
 *
 * قبلاً `new Set(['card', 'tap'])` بود و معرف از ثبتِ کارت و بازیِ
 * ضربه‌زنِ کاربرِ دعوت‌شده امتیاز می‌گرفت. حالا تنها مسیرِ زندهٔ
 * کمیسیون `payPurchaseCommission` است که روی خریدِ **نهایی‌شده**
 * (`shop_item` | `plus_monthly` | `plus_annual`) کار می‌کند و پول
 * واقعی به کیف پول می‌ریزد.
 *
 * چرا مجموعهٔ خالی به‌جای حذفِ کاملِ تابع: مسیرِ امتیازی ممکن است
 * روزی برگردد و نگه‌داشتنِ ساختار با یک لیست سفیدِ خالی، هم قصد را
 * صریح می‌کند هم تستِ «هیچ منبعی کمیسیون نمی‌سازد» را ممکن.
 *
...
/**
 * کمیسیونِ **امتیازی** — در حال حاضر همیشه `null` برمی‌گرداند.
 *
 * ⚠️ این تابع عملاً غیرفعال است چون `COMMISSIONABLE` خالی است. مالک
 * تصمیم گرفت کمیسیون ۵٪ فقط از **خریدِ کامل‌شدهٔ شاپ** بیاید، نه از
 * امتیازِ بازی/تپ/کارت. برای کمیسیونِ زنده به `payPurchaseCommission`
 * نگاه کنید.
 *
 * منطقِ داخلی دست‌نخورده ماند تا اگر روزی مسیرِ امتیازی برگشت، فقط
 * کافی باشد منبع به `COMMISSIONABLE` اضافه شود — و همهٔ محافظ‌های
 * قبلی (امتیاز مثبت، معرفِ فعال، ثبت در دفتر و لیگ) سرِ جایشان باشند.
 *
