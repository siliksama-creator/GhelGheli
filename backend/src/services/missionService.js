...
// ماموریتِ اختصاصیِ ادمین — یک تنظیمِ یگانه که بالای «ماموریت‌های امروز»
// به همهٔ کاربران نشان داده می‌شود (خواستهٔ مالک، ۱۷ شهریور).
const customMission = require('./customMission');
...
const DAILY_FAMILIES = Object.freeze([
  ...
  {
    // ═════════════════════════════════════════════════════════════════════
    // دورِ ۳۴ (۱۷ شهریور) — «ماموریتِ دعوتِ دوستان»
    // ═════════════════════════════════════════════════════════════════════
    //
    // ── چرا اضافه شد ────────────────────────────────────────────────────
    //
    // خواستهٔ مالک: «این ماموریتِ دعوتِ دوستان … بررسی کن که آیا واقعاً
    // امتیاز می‌گیرد.» بررسی نشان داد **چنین ماموریتی وجود نداشت**: دعوتِ
    // یک دوست، چرخشِ گردونه و کمیسیونِ خرید می‌داد ولی **صفر امتیاز**. تنها
    // ماموریتِ اجتماعیِ موجود `friend_challenge` بود — «دعوت به یک دوئل»،
    // نه «دعوت به قلقلی». پس نه باگی در پرداخت بود و نه چیزی که کاربر
    // بتواند در دفترِ امتیازش ببیند.
    //
    // ── چرا روزانه و با بالاترین پایه ──────────────────────────────────
    //
    // آوردنِ یک آدمِ تازه سخت‌ترین کارِ اجتماعیِ محصول است (باید کسی را
    // متقاعد کنی ثبت‌نام کند)، پس پایه‌اش ۴۵ است — بالاتر از همه، از جمله
    // `friend_challenge` (۳۰). هدف‌ها هم محافظه‌کارانه‌اند (۱ تا ۳ دعوتِ
    // موفق در روز) چون هر دعوت یک کاربرِ واقعیِ تازه است، نه یک کلیک.
    //
    // ── رویداد ─────────────────────────────────────────────────────────
    //
    // `referral_signup` — در `auth.js` **بعد از COMMIT** و برای «معرف»
    // ثبت می‌شود (نه دعوت‌شده). اگر داخلِ تراکنش بود، خطای بعدی می‌توانست
    // پیشرفتِ ماموریت را بدونِ ثبت‌نامِ واقعی جا بگذارد.
    event: 'referral_signup', icon: 'handshake', baseReward: 45,
    titles: ['دعوت اول', 'دوست تازه', 'قلقلی را معرفی کن',
      'سفیر محله', 'پخش‌کنندهٔ خبر', 'تیم‌ساز'],
    describe: goal => `${goal} دوستِ تازه را به قلقلی دعوت کن`,
    goals: [1, 1, 1, 2, 2, 3],
  },
]);
...
async function activeDefinitions(userId, now = new Date()) {
  ...
  // ── پنج ماموریت از پنج خانوادهٔ **متمایز** در روز ─────────────────────
  //
  // قاعدهٔ محصول («هر روز ۵ ماموریت تازه») با اضافه شدنِ خانوادهٔ ششم
  // («دعوتِ دوستان»، دورِ ۳۴) عوض نشد؛ فقط حالا از میان **شش** خانواده
  // پنج‌تا انتخاب می‌شود — بر اساسِ رتبهٔ هشِ (کاربر، روز، خانواده).
  // دو دلیل که چرا نمی‌شود همهٔ خانواده‌ها را هر روز داد:
  //
  //   • «۵ ماموریت» در سه جا خوانده می‌شود: متنِ UI، `dailyBonus.goal` و
  //     شرطِ جایزهٔ تکمیلِ روزانه. شش‌تایی‌کردن یعنی کاربرِ روزِ اول با
  //     «۵/۶» و جایزهٔ نگرفته روبه‌رو می‌شد.
  //   • تنوع: هر روز یک خانواده بیرون می‌ماند تا فردا تازه باشد.
  const dailyFamilies = DAILY_FAMILIES.slice()
    .sort((a, b) => hashRank(`${userId}:${day}:fam:${a.event}`)
      .localeCompare(hashRank(`${userId}:${day}:fam:${b.event}`)))
    .slice(0, 5);
  const daily = dailyFamilies.map(family => DAILY_POOL
    .map(applyOverride)
...
async function status(userId) {
  ...
  return {
    ...
    // ── ماموریتِ اختصاصیِ ادمین ─────────────────────────────────────────
    // در خروجیِ **وضعیت** می‌آید (نه به‌عنوانِ یک ماموریتِ روزانه) چون:
    //   • «یک‌بار برای هر دوره» است، نه چرخشِ روزانه، پس نباید در
    //     `daily` بنشیند و جای یکی از پنج ماموریتِ روز را بگیرد؛
    //   • کلاینت باید بتواند آن را **جدا و بالاتر** از «ماموریت‌های امروز»
    //     بکشد — همان جایی که مالک خواست.
    // اگر ادمین فعالش نکرده باشد `null` می‌شود و هیچ چیزی رندر نمی‌شود.
    //
    // ⚠️ این کلید **می‌ماند**: نسخهٔ نصب‌شدهٔ اندروید فقط `custom` را
    //    می‌شناسد. حذفش یعنی کاربرانِ همان APK یا کارتِ ماموریت را از دست
    //    بدهند یا دکمهٔ دریافتشان بشکند.
    custom: await customMission.status(userId).catch(() => null),
    // ── فهرستِ ماموریت‌های اختصاصی (چند ماموریتی) ──────────────────────
    // خواستهٔ مالک (۲۹ شهریور): «چند تا ماموریتِ اختصاصی بشود قرار داد».
    // وب و اندرویدِ تازه از این فهرست کارت می‌سازند؛ کلیدِ `custom` بالا
    // فقط برای سازگاری با کلاینتِ قدیمی نگه داشته شده.
    customMissions: await customMission.statuses(userId).catch(() => []),
    dailyBonus: {
      key: DAILY_BONUS_KEY,
