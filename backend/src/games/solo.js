  ...
  io.on('connection', socket => {
    ...
    socket.on('solo:move', payload => {
      try {
        ...
        run.done = true;
        // ── چرا کفِ ۱ میلی‌ثانیه ──
        //
        // `Date.now()` دقتِ میلی‌ثانیه دارد. اگر بازی در همان
        // میلی‌ثانیه‌ای تمام شود که شروع شده — که روی ماشینِ سریع یا
        // در تستِ خودکار کاملاً ممکن است — اختلاف **صفر** می‌شود.
        //
        // صفر به کاربر نشان داده می‌شود («۰ ثانیه») و در رکوردها هم
        // می‌نشیند: زمانی که از نظر فیزیکی غیرممکن است.
        //
        // CI این را گرفت، نه تستِ محلی: رانرِ گیت‌هاب سریع‌تر از
        // ماشینِ توسعه بود و تستِ «duration is measured server-side»
        // قرمز شد. باگِ وابسته به سرعتِ ماشین که فقط گاهی ظاهر
        // می‌شود، بدترین نوع است.
        //
        // ⚠️ این کف جای `MIN_PLAUSIBLE_MS` را نمی‌گیرد: آن جلوی
        //    **ذخیرهٔ** رکوردِ غیرممکن را می‌گیرد، این فقط عددِ
        //    نمایشی را معنادار می‌کند.
