...
function applyMove(state, i, sym) {
  ...
  if (state.flipped.length === 2) {
    ...
    if (state.deck[a] === state.deck[b]) {
      ...
      // کارت‌ها در state باز می‌مانند تا موتور بعد از missHoldMs خودش
      // برشان گرداند (یا حرکتِ بعدی زودتر پاکشان کند). خواستهٔ مالک:
      // جفتِ ناموفق نباید تا نوبتِ حریف باز بماند.
      state.keepTurn = null;
    }
  } else {
    state.lastResult = null;
...
module.exports = {
  ...
  // «وقتی دو تا عکس رو برمی‌گردونه و می‌بینه شبیه هم نیستن دیگه نباید
  // اون دو تا عکس همین‌طوری بمونن و قابل دیدن باشن تا نوبتِ فردِ مقابل —
  // باید زود دوباره برگردن.» لایهٔ قوانین جفت را در `flipped` نگه
  // می‌دارد (هر دو طرف باید ببینندش) و موتور بعد از این تأخیر خودش
  // کارت‌ها را می‌بندد و تخته را دوباره پخش می‌کند. ۱.۱ ثانیه برای
  // دیدنِ جفت کافی است و بازی را کش نمی‌دهد.
  missHoldMs: 1100,
  // ── تمرین با ربات = فقط رکوردی (خواستهٔ مالک، ۳۱ شهریور ۱۴۰۵) ──────
  // «حالت تمرین به ربات رو هم باید فقط حالت رکوردی کرد.» امتیاز/XP که
  // از قبل صفر بود (recordMatch ورودی vsBot را رد می‌کند و XP پشتِ شرطِ
  // ورودی‌دار است)؛ با این پرچم موتور حتی ماموریتِ bot_match را هم برای
  // اتاقِ رباتِ جفت‌یاب ثبت نمی‌کند — تمرین، تمرین می‌ماند.
  practiceOnlyBot: true,
