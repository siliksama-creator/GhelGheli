...
module.exports = {
  ...
  //
  // اعلانِ سینمایی ۲ ثانیه روی صفحه است؛ ۲.۶ ثانیه داده شد تا حتی
  // کندخوان‌ها هم شعار و راهنمای راند را کامل ببینند و بعد ساعت شروع
  // شود. موتور این عدد را به `deadline` اضافه می‌کند و کلاینت تا
  // `introUntil` شمارش را نگه می‌دارد.
  //
  // ⚠️ اگر مدتِ انیمیشن در کلاینت عوض شد، این هم باید عوض شود. نگهبان:
  //    `scripts/testCardDuelIntroClock.js`.
  introMs: 2600,
  ...
  //
  // اندازه‌گیریِ زنده نشان داد فازِ اعلامِ برنده فقط ۴ ثانیه دوام
  // می‌آورد و بعد اوورلیِ راندِ بعدی رویش می‌افتد؛ و راندِ دوم به بعد
  // فقط ۵٫۲ ثانیه کل عمر دارند.
  //
  // انیمیشنِ کلاینت این‌طور تقسیم شده:
  //     charge ۰٫۶s + impact ۰٫۴s + numbers ۰٫۹s + verdict ۱٫۶s = ۳٫۵s
  // با ۳۸۰۰ms، کاربر کلِ نمایش را می‌بیند و ~۰٫۳ ثانیه هم مکثِ خواندن
  // دارد، بعد اعلانِ راندِ بعد شروع می‌شود.
  //
  // ⚠️ اگر مدتِ فازهای کلاینت عوض شد، این هم باید عوض شود. نگهبان:
  //    `scripts/testCardDuelPacing.js`.
  resultHoldMs: 3800,
  simultaneous: true,
  create,
  createFromDecks,
