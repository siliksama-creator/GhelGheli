...
function armTurnClock(room) {
  ...
  const holdBase = room.state && room.state.lastRound
    ...
    : 0;
  // ── مکثِ بلندتر برای وقتِ اضافه (خواستهٔ مالک، ۸ مهر ۴۰۵) ─────────
  // گزارشِ مالک دربارهٔ تساویِ راندِ دوامتیازی: «راند شش بدون هیچ اکشنی
  // و توضیحی سریع تموم میشه». وقتِ اضافه خودکار داوری می‌شود (قدرتِ کلِ
  // ترکیب)، پس تنها فرصت برای *دیده و فهمیده شدن* همین مکث است. قواعد
  // بازی `otHoldMs` اعلام می‌کند و موتور فقط وقتی آن را به کار می‌برد
  // که راندِ قبل واقعاً به وقت اضافه کشیده باشد.
  ...
  io.on('connection', socket => {
    ...
    socket.on('game:create_room', async (payload, callback) => {
      ...
      const response = {
        roomCode: code,
        gameId,
        shareUrl: `${(process.env.PUBLIC_WEB_URL || process.env.CLIENT_URL || 'https://user.ghelghelishop.com').replace(/\/$/, '')}/?game=${gameId}&room=${code}`,
