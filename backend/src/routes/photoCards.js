...
module.exports = function createPhotoCardRoutes(deps) {
  ...
  /**
   * پیش‌چکِ سبکِ کد — بدون عکس و بدون پردازش تصویر.
   *
   * ═══════════════════════════════════════════════════════════════════════
   * چرا این endpoint لازم است
   * ═══════════════════════════════════════════════════════════════════════
   *
   * در اپِ گوشی، جریانِ ثبت ابتدا مدلِ عصبیِ روی‌گوشی را روی عکس اجرا می‌کند
   * (چند ثانیه CPU) و **بعد** کد را می‌فرستد. اگر کد غلط یا قبلاً مصرف‌شده
   * باشد، کاربر بی‌دلیل چند ثانیه منتظر تشخیص می‌ماند. این endpoint فقط کد را
   * با یک کوئریِ ارزانِ ایندکس‌شده می‌سنجد تا اپ **قبل از اجرای مدل** سریع
   * نتیجه را بگوید.
   *
   * ═══════════════════════════════════════════════════════════════════════
   * ضدتقلب/حدسِ کد (code enumeration)
   * ═══════════════════════════════════════════════════════════════════════
   *
   * این endpoint هرگز «کدِ معتبر و آماده» را آشکار نمی‌کند؛ وگرنه مهاجم با
   * امتحابِ انبوه کدها، کدهای استفاده‌نشده را پیدا و عکسِ جعلی می‌فرستاد.
   *   • کد اصلاً در سیستم نیست یا بدشکل است   → ok:false، علت «در سیستم ثبت
   *     نشده» (همان پیامِ submit) — این خودش اطلاعاتِ مفید به کاربرِ واقعی و
   *     بی‌فایده به مهاجم است، چون کدِ روی کارت باید معتبر باشد.
   *   • کد مصرف‌شده/رزرو/باطله               → ok:false با علت.
   *   • کد معتبر و آمادهٔ مصرف                 → ok:true، ولی هیچ اطلاعاتی
   *     دربارهٔ نوع کارت/امتیاز نمی‌دهد؛ ثبتِ واقعی همچنان عکس می‌خواهد.
   *
   * این endpoint چیزی را رزرو یا تغییر نمی‌دهد (نه شمارندهٔ خطا، نه وضعیت کد)
   * تا فراخوانیِ تکراریِ اپ بی‌عارضه باشد؛ شمارشِ خطا فقط در submit می‌ماند.
   */
  router.get('/photo-cards/check-code', auth, asyncHandler(async (req, res) => {
    const code = photoCards.normalizePhotoCode(req.query.code);
    if (!photoCards.isValidPhotoCode(code)) {
      return res.json({ ok: false, reason: 'bad_format',
        message: 'کدِ واردشده معتبر نیست.' });
    }
    const { rows } = await pool.query(
      `SELECT status FROM photo_card_codes WHERE code_fold=$1`,
      [photoCards.foldPhotoCode(code)],
    );
    // تصمیم در تابع خالصِ سرویس (classifyCodeStatus) تا تست‌پذیر بماند.
    const result = photoCards.classifyCodeStatus(rows[0] ? rows[0].status : null);
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if (decision.action !== 'approve') {
          ...
          //
          // عکسِ کامل روی دیسک مانده. کمی بعد (تا پاسخ کاربر برود) سرور با
