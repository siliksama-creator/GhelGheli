...
(async () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  //
  // خواستهٔ مالک: «هر لول که می‌گیرن ۵ سکه هم دریافت می‌کنن».
  // عدد از `gameEconomyService.tapCoinsPerLevel` می‌آید (قابل تنظیم
  // توسط ادمین) و پیش‌فرضش ۵ است.
  ok(tapLevelCoin(1) === 5, 'لول ۱ → ۵ سکه');
  ok(tapLevelCoin(5) === 5, 'لول ۵ → ۵ سکه');
  ok(tapLevelCoin(6) === 5, 'لول ۶ → ۵ سکه');
  ok(tapLevelCoin(46) === 5, 'لول ۴۶ → ۵ سکه');
  ok(tapLevelCoin(50) === 5, 'لول ۵۰ → ۵ سکه');
  ...
  {
    // جمعِ کلِ مسیر باید دقیقاً ۲۵۰ باشد (۵۰ لول × ۵ سکه).
    let total = 0;
    for (let lv = 1; lv <= 50; lv++) total += tapLevelCoin(lv);
    ok(total === 250, `جمعِ کلِ ۵۰ لول ۲۵۰ سکه است (${total})`);
  }

  // جمعِ یک بسته: سرویس فهرستِ لول‌های تمام‌شده را می‌دهد، نه اختلافِ ساده.
  ok(tapCoinsFor([9, 10, 11]) === 15, 'بستهٔ لول ۹ و ۱۰ و ۱۱ → ۱۵ سکه');
  ok(tapCoinsFor([]) === 0, 'بستهٔ خالی صفر سکه');
  ok(tapCoinsFor(null) === 0, 'ورودیِ غیرآرایه صفر سکه');
  ok(tapCoinsFor([50, 50]) === 10, 'تکرارِ لول جمع می‌شود (منطقِ خالصِ جمع)');
  ...
  ok(tapLevelCoin(50) < coinRewardFor('card_duel', 1000).win,
    'سکهٔ یک لولِ ضربه‌زن از بردِ یک دوئلِ ۱۰۰۰ کمتر است');
