...
const PUBLIC_OK = new Map([
  // «برنامه‌های پیشنهادی» — ورودیِ صفحهٔ «بیشتر» در وب و اندروید.
  // عمداً عمومی است: منو پیش از آماده‌شدنِ ورود/توکن رندر می‌شود و اگر این
  // مسیر احراز هویت می‌خواست، بخش در اولین ثانیه‌های باز شدنِ اپ غایب
  // می‌شد. محتوایش هم چیزِ حساسی نیست (عنوان/توضیح/لینکِ معرفی).
  ['GET /api/recommended-apps', 'ورودیِ صفحهٔ «بیشتر» — محتوای عمومیِ معرفی'],
  // شماره معکوسِ شروعِ لیگ — سه صفحهٔ لیگ/بازیِ آنلاین/ضربه‌زن آن را
  // می‌خوانند. عمومی است چون کارتِ شمارش باید در **اولین** رندر بیاید و
  // منتظرِ توکن نمی‌ماند؛ محتوایش هم فقط متن و زمان است (هیچ دادهٔ کاربری).
  ['GET /api/league/countdown', 'کارتِ شماره معکوس در سه صفحه — متن و زمان، بدونِ دادهٔ کاربری'],
  ...
  // پیکربندی عمومی کلاینت: فقط minVersion/بنر/پرچم‌های مشتق‌شده —
  // هیچ دادهٔ کاربری ندارد و محدودکنندهٔ نرخ هم دارد.
  ['GET /api/config', 'پیکربندی عمومی کلاینت — بدونِ دادهٔ کاربر'],
  // فهرستِ آواتارها هم عمومی است: فقط نامِ فایل‌ها را می‌دهد، هیچ دادهٔ
  // کاربری در آن نیست. بدونِ این ورودی، گاردِ فهرستِ سفید درست ایستاد —
  // یعنی اگر روزی کسی دادهٔ حساسی به این مسیر اضافه کند، همین تست قرمز
  // می‌شود و مجبور می‌کند دلیلِ عمومی‌بودن را دوباره بنویسیم.
  ['GET /api/avatars', 'فهرستِ آواتارهای باندل — بدونِ دادهٔ کاربر'],

  // گزارشِ کرش عمداً مهمان‌پذیر است (authOptional): مهم‌ترین خطاها همان‌هایی
  // هستند که پیش از ورود/ثبت‌نام روی اولین تجربهٔ کاربر رخ می‌دهند و اگر
  // توکن لازم بود برای همیشه گم می‌شدند. ورودی در خودِ سرویس پاک‌سازی و
  // سقف‌دار می‌شود (safeObject/cleanText، رمز/موبایل/توکن redact)، با
  // crashLimiter بر پایهٔ IP هم محدود است و هیچ داده‌ای برنمی‌گرداند
  // (فقط 202 + شناسهٔ گزارش). توکنِ معتبر اگر حاضر باشد شناسهٔ کاربر را
  // می‌نشاند.
  ['POST /api/telemetry/crash',
    'گزارشِ خطا — مهمان‌پذیرِ عمدی؛ ورودی پاک‌سازی می‌شود، سقفِ نرخ بر IP دارد، چیزی برنمی‌گرداند'],
  // زرین‌پال کاربر را بدون توکن به این کال‌بک برمی‌گرداند؛ Authority فقط
  // کلیدِ جست‌وجوی سفارش است و مبلغ/تحویل سمت سرور verify می‌شود.
  ['GET /api/payments/zarinpal/callback',
    'کال‌بک عمومی زرین‌پال — فقط سفارشِ pending متناظر verify و تحویل می‌شود'],
]);
