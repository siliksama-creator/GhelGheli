...
const PUBLIC_OK = new Map([
  ...
  // «آخرین نسخهٔ اپ» — چکِ به‌روزرسانی در وب و اندروید، و صفحهٔ دانلود.
  // عمداً عمومی است: کلاینت پیش از ورود هم باید بفهمد نسخهٔ تازه‌ای هست یا
  // نه، و صفحهٔ دانلود هم بدونِ حساب کاربری باید لینک و هشِ SHA-256 را
  // نشان بدهد (کاربر باید بتواند از سالم‌بودنِ فایل مطمئن شود). محتوایش
  // هیچ دادهٔ کاربری ندارد: نسخه، حجم، تاریخ، هش و آدرسِ دانلودِ خودمان.
  // ⚠️ مسیرِ آپلود/حذف/فهرست زیرِ `/api/admin/apk` است و همان‌جا
  //    `adminAuth` + `requireRole()` دارد؛ فقط خواندنِ نسخه عمومی است.
  ['GET /api/app/latest', 'چکِ به‌روزرسانی در وب/اپ و صفحهٔ دانلود — نسخه/هش/لینک، بدونِ دادهٔ کاربری'],
  ...
  // فایلِ صوتیِ آموزش (تورِ بعدِ ورود) — عمداً عمومی: عنصرِ `<audio>`
  // مرورگر هدرِ سفارشی نمی‌فرستد، پس توکن اصلاً به این مسیر نمی‌رسد و
  // گذاشتنِ auth یعنی تور در وب همیشه بی‌صدا. نامِ فایل از فهرستِ سفیدِ
  // سرویس می‌آید (پیمایشِ مسیر ممکن نیست) و محتوا فقط فایل‌های ثابتِ
  // آموزشی است، بدونِ هیچ دادهٔ کاربری.
  ['GET /api/onboarding/audio/:file',
    'پخشِ فایلِ صوتیِ آموزش — عنصرِ audio هدرِ توکن نمی‌فرستد؛ نام از فهرستِ سفید'],
