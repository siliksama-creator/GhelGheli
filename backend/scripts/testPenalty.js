...
console.log('\n== ساختار اولیه ==');
{
  ...
  ok(s.suddenDeath === false, 'مرگِ ناگهانی وجود ندارد (فیلدِ سازگاری، همیشه false)');
  ok(P.ZONES === 9, 'دروازه ۹ ناحیه دارد');
  ok(P.ROUNDS === 10, 'هر بازیکن ۱۰ ضربه می‌زند (خواستهٔ مالک، ۳ مهر ۱۴۰۵)');
...
console.log('\n== پایان مسابقه ==');
{
  ...
  // با ۱۰ ضربه، در ۶-۰ (یعنی ۱۲ ضربه) کار تمام است چون O فقط ۴ ضربه دارد.
  ok(kicks < 20, `برد زودهنگام: بازی در ${kicks} ضربه تمام شد نه ۲۰`);

  // ── تساوی: ۱۰ ضربه برای هرکس، همه گل ──
  // این همان سناریویی است که مالک گفت: «هر وقت نفری ۱۰ ضربه زدن و بازی
  // مساوی شد باید مساوی حساب بشه». پیش از این، همین‌جا بازی برای همیشه
  // به مرگِ ناگهانی می‌رفت و هرگز تمام نمی‌شد.
  const s2 = P.create();
  for (let i = 0; i < 20; i++) kick(s2, true);
  ok(s2.taken.X === 10 && s2.taken.O === 10, 'هر کس دقیقاً ۱۰ ضربه زد');
  ok(P.result(s2) === 'DRAW', 'امتیاز برابر بعد از ۱۰ ضربه → مساوی');
  ok(s2.suddenDeath === false, 'مرگِ ناگهانی هرگز روشن نمی‌شود');

  // ── نصفه‌کاره: تا وقتی ضربهٔ دهمِ هر دو نرسیده، بازی تمام نیست ──
  const s3 = P.create();
  for (let i = 0; i < 16; i++) kick(s3, true);   // ۸ ضربه هرکس، مساوی
  ok(P.result(s3) === null, 'مساویِ وسطِ بازی نتیجه اعلام نمی‌کند');
  for (let i = 0; i < 4; i++) kick(s3, true);    // ضربه‌های ۹ و ۱۰
  ok(P.result(s3) === 'DRAW', 'و در پایان همان مساوی می‌شود');

  // ── برندهٔ امتیازِ بیشتر در ضربهٔ دهم ──
  const s4 = P.create();
  for (let i = 0; i < 20; i++) kick(s4, i % 9 !== 0);  // X یک ضربه را خراب می‌کند
  ok(s4.taken.X === 10 && s4.taken.O === 10, 'بازی تا پایان کشیده شد');
  ok(P.result(s4) === P.result(s4) && P.result(s4) !== null,
    'در پایانِ وقت برنده مشخص می‌شود یا مساوی اعلام می‌شود');
}
