...
async function testAuth() {
  ...
  // سیاستِ ۳۰ شهریور ۱۴۰۵ (دستورِ مالک): ورود با رمز برای هر حسابی که
  // **از قبل رمز دارد** باز است (اکانت‌های تستِ قبلی + مدیر)؛ ثبت‌نام با رمز
  // همچنان بسته است. پس «رمزِ اصلی سالم است» یعنی رمزِ درست حالا ۲۰۰ با
  // توکن می‌گیرد، و رمزِ مهاجم باید همچنان ۴۰۱ بگیرد (یعنی بازنویسی نشده).
  ...
  ok(stillWorks.status === 200 && stillWorks.data?.token,
    'رمز اصلی حسابِ دارای رمز دست‌نخورده است و ورود با آن باز است (سیاستِ ۳۰ شهریور)',
