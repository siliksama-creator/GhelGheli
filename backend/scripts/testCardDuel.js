...
const mobile = read('mobile/lib/screens/user/games/card_duel_page.dart')
  + ['card_duel_battle.dart', 'card_duel_scoreboard.dart', 'card_duel_clash.dart', 'card_duel_payout.dart', 'card_duel_focus.dart', 'card_duel_intro.dart'].map((f) => read('mobile/lib/screens/user/games/card_duel/' + f)).join('\n');
...
// از دور ۲۱ عبارت «۱۰٪ کارمزد» از صفحهٔ بازی حذف شد و افشای ریسک به شکل
// «بازنده N امتیاز می‌دهد» بازنویسی شد؛ شرط همچنان همان است: پیش از ورود
// باید روشن باشد که باخت امتیاز کم می‌کند.
const risk = /بازنده\s*\$?\{?\s*(fa|faNum)\(/;
ok(risk.test(mobileHub) && /امتیاز می‌دهد/.test(mobileHub)
  && risk.test(webHub) && /امتیاز می‌دهد/.test(webHub),
