...
// ⚠️ این ادعا عمداً حذف شد و جایش را `scripts/testTurnClock.js` گرفت.
//
// نسخهٔ قبلی این بود:
//     /deadline\s*=\s*Date\.now\(\)\s*\+\s*holdMs/.test(engine)
//
// و یک باگِ واقعی را از دست داد: خطِ `deadline` درست بود (پس regex سبز
// می‌شد) ولی آرگومانِ `setTimeout` خطِ بعد `holdMs` را نداشت، یعنی
// نوبتِ کاربر ۳۲۰۰ms زودتر از ساعتِ رویِ صفحه‌اش می‌سوخت. یک تستِ
// متن‌محور نمی‌تواند بینِ «فرمول نوشته شده» و «فرمول اجرا می‌شود»
// تفاوت بگذارد.
//
// `testTurnClock.js` همان قرارداد را رفتاری چک می‌کند: موتور را با
// تایمرِ جعلی می‌رانَد و تأیید می‌کند `deadline` و آرگومانِ واقعیِ
// `setTimeout` دقیقاً یکی‌اند. اینجا دوباره ادعا نمی‌کنیم.
